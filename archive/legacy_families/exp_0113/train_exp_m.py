"""
exp_0113/train_exp_m.py: Exp M - Adapter + LoRA 混合去噪訓練

核心設計:
- 淺層 (L2, L4): 大 Adapter → 激進去噪
- 中層 (L6): 小 Adapter + 小 LoRA → 細化去噪
- 深層 (L9-L16): 微量 LoRA → 輕微調整

設計理念:
- 結合 Adapter 的聽感優勢 (Exp J 發現)
- 加入 LoRA 補足數值性能 (Exp I 證明 LoRA 有效)
- 分層設計: 淺層重去噪、深層輕調整

目標參數量: ~200K-300K
- Adapter: ~70K (與 Exp L 類似)
- LoRA: ~150K-230K (比 Exp I 的 3.7M 小很多)

執行:
    python exp_0113/train_exp_m.py --exp_name exp_m_adapter_lora
"""
...
from exp_0113.models import TeacherStudentAdapterLoRA
...
def verify_model_state(model, stage: str):
    ...
    Args:
        model: TeacherStudentAdapterLoRA 模型
        stage: 當前階段描述字串
...
def plot_metrics(history, exp_dir):
    ...
    plt.suptitle('Exp M: Adapter + LoRA Hybrid Denoising', fontsize=14)
    plt.tight_layout()
...
def main():
    ...
    執行 Exp M: Adapter + LoRA 混合去噪訓練
    """
    parser = argparse.ArgumentParser(description='Exp 0113: Adapter + LoRA 混合去噪 (Exp M)')

    # Experiment
    parser.add_argument('--exp_name', type=str, default='exp_m_adapter_lora')
    parser.add_argument('--output_dir', type=str, default=None)

    # Shallow Adapter Config (L1, L4)
    parser.add_argument('--shallow_adapter_positions', type=str, default='1,4',
                        help='淺層 Adapter 位置 (ResBlock 後)')
    parser.add_argument('--shallow_reduction', type=int, default=2,
                        help='淺層 Adapter 降維倍數')

    # Mid Adapter Config (L7)
    parser.add_argument('--mid_adapter_positions', type=str, default='7',
                        help='中層 Adapter 位置 (ResBlock 後)')
    parser.add_argument('--mid_reduction', type=int, default=4,
                        help='中層 Adapter 降維倍數')

    # Mid LoRA Config (L4, L7 ResBlock)
    parser.add_argument('--mid_lora_layers', type=str, default='4,7',
                        help='中層 LoRA 層 (ResBlock)')
    parser.add_argument('--mid_lora_rank', type=int, default=32,
                        help='中層 LoRA rank')

    # Deep LoRA Config (L10, L13, L16 ResBlock)
    parser.add_argument('--deep_lora_layers', type=str, default='10,13,16',
                        help='深層 LoRA 層 (ResBlock)')
    parser.add_argument('--deep_lora_rank', type=int, default=16,
                        help='深層 LoRA rank')

    # Common Config
    parser.add_argument('--dropout', type=float, default=0.1)
    parser.add_argument('--init_scale', type=float, default=0.01,
                        help='Adapter 初始 scale')

    # Learning Rate
    parser.add_argument('--lr', type=float, default=1e-4,
                        help='Learning rate for all trainable params')
    ...
    # 解析配置
    shallow_adapter_positions = [int(x) for x in args.shallow_adapter_positions.split(',')]
    mid_adapter_positions = [int(x) for x in args.mid_adapter_positions.split(',')]
    mid_lora_layers = [int(x) for x in args.mid_lora_layers.split(',')]
    deep_lora_layers = [int(x) for x in args.deep_lora_layers.split(',')]
    ...
    config['experiment_type'] = 'Exp M: Adapter + LoRA Hybrid Denoising'
    config['shallow_adapter_positions_list'] = shallow_adapter_positions
    config['mid_adapter_positions_list'] = mid_adapter_positions
    config['mid_lora_layers_list'] = mid_lora_layers
    config['deep_lora_layers_list'] = deep_lora_layers
    ...
    print("=" * 60)
    print(f"\nHybrid Architecture:")
    print(f"  Shallow Adapter: {shallow_adapter_positions} (reduction={args.shallow_reduction})")
    print(f"  Mid Adapter: {mid_adapter_positions} (reduction={args.mid_reduction})")
    print(f"  Mid LoRA: {mid_lora_layers} (rank={args.mid_lora_rank})")
    print(f"  Deep LoRA: {deep_lora_layers} (rank={args.deep_lora_rank})")
    print(f"  LR: {args.lr}")
    print("=" * 60)
    ...
    # Create model
    print("\n創建模型...")
    model = TeacherStudentAdapterLoRA(
        ...
        shallow_reduction=args.shallow_reduction,
        mid_reduction=args.mid_reduction,
        mid_lora_rank=args.mid_lora_rank,
        deep_lora_rank=args.deep_lora_rank,
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if val_metrics['masked_acc'] > best_val_acc:
            ...
            # 保存 Student encoder 的狀態
            torch.save({
                'epoch': epoch,
                'student_encoder_state_dict': model.student.feature_extractor.encodec.encoder.state_dict(),
