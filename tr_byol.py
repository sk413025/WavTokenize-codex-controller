...
class SimpleTransformer(nn.Module):
    def __init__(self, input_dim, hidden_dim, projection_dim, num_heads, num_layers):
        ...
        self.fc = nn.Linear(hidden_dim, 256)  # 確保輸出為 256 維
...
class BYOL(nn.Module):
    def __init__(self, input_dim, hidden_dim, projection_dim, num_heads, num_layers):
        super().__init__()
        self.online_encoder = SimpleTransformer(input_dim, hidden_dim, projection_dim, num_heads, num_layers)
        self.target_encoder = SimpleTransformer(input_dim, hidden_dim, projection_dim, num_heads, num_layers)
        self.online_projector = ProjectionMLP(256, hidden_dim, projection_dim)  # 修改輸入維度
        self.target_projector = ProjectionMLP(256, hidden_dim, projection_dim)  # 修改輸入維度
...
def train_byol(model, optimizer, dataloader, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in dataloader:
            ...
            with torch.no_grad():
                # Ensure the output of the encoder has the correct shape
                encoder_output = model.online_encoder(x1)
                embeddings.append(encoder_output.cpu().numpy())
