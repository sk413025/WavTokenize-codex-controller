"""
exp_1219: 支援 Cosine Similarity Loss 的訓練腳本

基於 exp_1217/train.py，新增:
1. Cosine Similarity Loss (解決特徵方向不對齊問題)
2. 記錄 cos_sim_mean/std 指標
"""
...
def main():
    ...
    parser.add_argument('--cosine_weight', type=float, default=0.0)  # NEW
    ...
    print("=" * 60)
    print(f"exp_1219: Cosine Similarity Loss 實驗")
    ...
    # Loss Function (使用 V2 版本，支援 cosine_weight)
    loss_fn = MaskedCombinedLossV2(
    ...
    history = {
        ...
        'train_cosine_loss': [], 'val_cosine_loss': [],  # NEW
        'train_cos_sim': [], 'val_cos_sim': [],  # NEW
        'train_dist': [], 'val_dist': [],
        'lr': [],
    }
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        print(f"\nTrain: Loss={train_metrics['total_loss']:.4f}, "
              ...
              f"Cos Sim={train_metrics['cos_sim_mean']:.3f}")
        print(f"Val:   Loss={val_metrics['total_loss']:.4f}, "
              f"Masked Acc={val_metrics['masked_acc']*100:.2f}%, "
              f"Cos Sim={val_metrics['cos_sim_mean']:.3f}")
        ...
        if val_metrics['masked_acc'] > best_val_acc:
            ...
            torch.save({
                ...
                'val_cos_sim': val_metrics['cos_sim_mean'],
                'config': config,
