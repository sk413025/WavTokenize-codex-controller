#!/bin/bash
# 監控 Exp I 完成後自動啟動 Exp K

echo "=========================================="
echo "Monitor: Exp I → Exp K"
echo "=========================================="
echo "Started at: $(date)"
echo ""

LOG_FILE="/home/sbplab/ruizi/WavTokenize-feature-analysis/exp_0112/exp_i.log"

# 等待 Exp I 完成
while true; do
    if grep -q "Training completed!" "$LOG_FILE" 2>/dev/null; then
        echo ""
        echo "[$(date)] Exp I completed!"
        break
    fi

    if grep -q "Error\|Exception\|Traceback" "$LOG_FILE" 2>/dev/null; then
        echo ""
        echo "[$(date)] Exp I encountered an error!"
        tail -20 "$LOG_FILE"
        exit 1
    fi
    ...
    if [ -n "$CURRENT" ]; then
        echo -ne "\r[$(date '+%H:%M:%S')] Exp I running: $CURRENT    "
    fi
