"""
Zero-Shot Speaker Denoising Transformer 快速驗證實驗

目的: 快速驗證 speaker conditioning 是否有效
配置:
- 2 speakers (boy1, girl9)
- 每人 10 sentences
...
使用方式:
    python train_zeroshot_quick.py \\
        ...
        --output_dir ./results/zeroshot_quick \\
        --num_epochs 5 \\
        --batch_size 4 \\
...
def train_epoch(model, dataloader, optimizer, criterion, device, epoch):
    ...
    for batch_idx, batch_data in enumerate(progress_bar):
        noisy_tokens = batch_data['noisy_tokens'].to(device)
        clean_tokens = batch_data['clean_tokens'].to(device)
        speaker_embeddings = batch_data['speaker_embeddings'].to(device)

        # Forward pass (zero-shot model with speaker conditioning)
        logits = model(noisy_tokens, speaker_embeddings, return_logits=True)  # (B, T, 4096)

        # Compute CE loss
        # Reshape: (B, T, vocab) -> (B*T, vocab) and (B, T) -> (B*T)
...
def main():
    parser = argparse.ArgumentParser(description='Zero-Shot Speaker Denoising - Quick Validation')

    # 數據參數
    parser.add_argument('--input_dirs', nargs='+', required=True, help='含噪音輸入目錄')
    parser.add_argument('--target_dir', required=True, help='乾淨目標目錄')
    parser.add_argument('--output_dir', default='./results/zeroshot_quick', help='輸出目錄')
    parser.add_argument('--max_sentences_per_speaker', type=int, default=10,
                       help='每個語者最多使用的句子數')
    ...
    parser.add_argument('--num_epochs', type=int, default=10, help='訓練 epochs')
    ...
    logger.info("=" * 80)
    logger.info("Zero-Shot Speaker Denoising Transformer - 快速驗證實驗")
    logger.info("=" * 80)
    ...
    audio_dataset = ZeroShotAudioDataset(
        ...
        allowed_speakers=['boy1', 'girl9']  # 只用 2 個 speakers
    )

    logger.info(f"總數據集大小: {len(audio_dataset)} 個音頻對")

    # 數據集分割（boy1 訓練，girl9 驗證）
    train_indices = []
    val_indices = []
    ...
    logger.info(f"訓練集 (boy1): {len(train_audio_dataset)} 樣本")
    logger.info(f"驗證集 (girl9): {len(val_audio_dataset)} 樣本")
    ...
    logger.info(f"  - 凍結參數: {frozen_params:,} ({frozen_params/total_params*100:.2f}%)")
    logger.info(f"    - Codebook: {codebook.numel():,}")
