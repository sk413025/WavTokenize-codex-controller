"""
families/eval/decoder_lora_eval/eval_0225d_0226.py

評估 exp_0225d 和 exp_0226（E2E）並更新 comparison_results.json 和 comparison_table.md：
  - exp_0225d: best_model.pt (epoch=14, val_wav_mse=0.02760; +FM loss, encoder from 0225a)
  - exp_0226:  best_model_val_total.pt (ep142, val_total=31.68) 和
               best_model.pt (ep51, val_wav_mse=0.02695; E2E LoRA)

執行：
    cd /home/sbplab/ruizi/WavTokenize-feature-analysis
    PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \
    /home/sbplab/miniconda3/envs/test/bin/python families/eval/decoder_lora_eval/eval_0225d_0226.py
"""
...
NEW_EXPERIMENTS = [
    {
        ...
        'folder': 'exp_0225d',
        'type': 'no_vq_dec_lora',
        'ckpt': 'families/deps/no_vq_scratch/runs/no_vq_scratch_dec_lora_fm_epoch_20260225_095842/best_model.pt',
        'lora_rank': 64, 'lora_alpha': 128,
        'decoder_lora_rank': 32, 'decoder_lora_alpha': 64,
        'note': 'best_model.pt (val_wav_mse criterion, epoch=14, val_wav_mse=0.02760; encoder from 0225a; +FM loss)',
    },
    {
        'name': 'exp_0226_best_total',
        'folder': 'exp_0226_best_total',
        'type': 'no_vq_e2e',
        'ckpt': 'families/deps/feat_align/runs/no_vq_e2e_epoch_20260225_101746/best_model_val_total.pt',
        'lora_rank': 64, 'lora_alpha': 128,
        'decoder_lora_rank': 32, 'decoder_lora_alpha': 64,
        'note': 'best_model_val_total.pt (epoch=142, val_total=31.68; E2E encoder+decoder LoRA)',
    },
    {
        'name': 'exp_0226_best_mse',
        'folder': 'exp_0226_best_mse',
        'type': 'no_vq_e2e',
        'ckpt': 'families/deps/feat_align/runs/no_vq_e2e_epoch_20260225_101746/best_model.pt',
        'lora_rank': 64, 'lora_alpha': 128,
        'decoder_lora_rank': 32, 'decoder_lora_alpha': 64,
        'note': 'best_model.pt (val_wav_mse criterion, epoch=51, val_wav_mse=0.02695; E2E encoder+decoder LoRA)',
    },
]
...
def run_no_vq_dec_lora(exp, loader, out_dir):
    ...
    return _infer(model, loader, out_dir, exp['name'])


def run_no_vq_e2e(exp, loader, out_dir):
    from families.deps.feat_align.models_no_vq_e2e import TeacherStudentNoVQE2E

    print(f"  Loading {exp['name']} (No-VQ E2E LoRA)...")
    ...
    ckpt = torch.load(str(BASE / exp['ckpt']), map_location='cpu', weights_only=False)
    # E2E checkpoint 格式：model_state_dict 或 trainable_state
    if 'model_state_dict' in ckpt:
        model.load_state_dict(ckpt['model_state_dict'], strict=False)
    else:
        # fallback: trainable params only
...
def _infer(model, loader, out_dir, name):
    metrics_list = []
    with torch.no_grad():
...
def rebuild_comparison_table(all_results):
    ...
    ORDER = [
        ...
        'V2', 'Plan_Ori', 'exp_0216', 'exp_0217', 'exp_0223_v2',
        'exp_0224a',
        'exp_0224b_best',
        'exp_0225a',
        'exp_0225b',
    ...
    LABELS = {
        ...
        'exp_0225c': 'exp_0225c (No-VQ+DecLoRA+Phase from 0225a, ep5) [負結果]',
        'exp_0225d': 'exp_0225d (No-VQ+DecLoRA+FM from 0225a, ep14)',
        'exp_0226_best_total': '**exp_0226_best_total** (E2E LoRA, ep142, best_val_total)',
    }
    ...
    lines.append("## 解讀說明")
    lines.append("")
    lines.append("- **PESQ(noisy) >> PESQ(recon)** 屬正常現象：LDV noisy 與 clean 時序高度對齊")
    ...
    lines.append("- **exp_0224b_best**（No-VQ+DecLoRA, encoder from 0224a, ep142）：有機械音")
    lines.append("- **exp_0225a/b/c/d**：以 0225a scratch encoder 為基礎的消融系列")
    lines.append("  - 0225b: decoder LoRA only")
    lines.append("  - 0225c: +phase loss（負結果，ep5 即最佳）")
    lines.append("  - 0225d: +frozen discriminator FM loss")
    lines.append("- **exp_0226**：E2E（encoder + decoder LoRA 同時訓練），理論最優架構")
...
def main():
    ...
    for exp in NEW_EXPERIMENTS:
        ...
        try:
            if exp['type'] == 'no_vq_dec_lora':
                metrics_list = run_no_vq_dec_lora(exp, loader, out_dir)
            elif exp['type'] == 'no_vq_e2e':
                metrics_list = run_no_vq_e2e(exp, loader, out_dir)
            else:
                raise ValueError(f"Unknown type: {exp['type']}")
