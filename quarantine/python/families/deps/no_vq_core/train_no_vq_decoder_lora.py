"""
exp_0224b: No-VQ + Decoder LoRA Fine-tune

架構：
    Encoder (Frozen, 繼承 exp_0217):
...
對比矩陣：
    exp_0223:   Encoder+VQ frozen → Decoder LoRA    (VQ tokens → decoder)
    exp_0224a:  Encoder LoRA 訓練 → VQ 跳過 → Decoder frozen
    exp_0224b:  Encoder frozen → VQ 跳過 → Decoder LoRA  ← 本腳本
    (未來)      Encoder LoRA + VQ 跳過 + Decoder LoRA（全端對端）
...
執行順序（必須先跑完 exp_0224a）：
    ...
    python families/deps/no_vq_core/train_no_vq_decoder_lora.py --mode smoke
"""
...
EXP0224A_BEST_CKPT_GLOB = 'families/deps/no_vq_core/runs/no_vq_epoch_*/best_model.pt'
...
def main():
    ...
    parser.add_argument('--encoder_ckpt', type=str, default=str(EXP0217_BEST_CKPT),
                        help='Encoder checkpoint 路徑。應指定 exp_0224a best_model.pt，'
                             '例如: families/deps/no_vq_core/runs/no_vq_epoch_*/best_model.pt')
    ...
    if args.output_dir:
        ...
        exp_dir = Path(f'families/deps/no_vq_core/runs/no_vq_dec_lora_{args.mode}_{timestamp}')
    ...
    config['experiment'] = 'exp_0224b_no_vq_decoder_lora'

    with open(exp_dir / 'config.json', 'w') as f:
        json.dump(config, f, indent=2)

    print("=" * 70)
    print("exp_0224b: No-VQ + Decoder LoRA")
    print("=" * 70)
    ...
    print(f"Encoder LoRA: rank={args.lora_rank} (FROZEN, from exp_0217)")
    print(f"VQ: SKIPPED")
