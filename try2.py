...
class EnhancedModel(nn.Module):
    ...
    def forward(self, x):
        ...
        return output
'''
    def extract_tsne_features(self, x):
        """提取用於t-SNE比較的特徵"""
        with torch.no_grad():
            features = self.feature_extractor.encodec.encoder(x)
            enhanced = self.feature_extractor(x)
...
def train_model(model, train_loader, optimizer, scheduler, device, save_dir, config):
    ...
    for epoch in range(config['epochs']):
        # 在每199個epoch或最後一輪時保存所有樣本
        if epoch % 199 == 0 or epoch == config['epochs'] - 1:  # 添加最後一輪的條件
        ...
        for batch_idx, (input_wav, target_wav) in progress_bar:
            try:
                ...
                try:
                    ...
                    # Save samples every 199 epochs
                    if epoch % 199 == 0 and batch_idx == 0:
                        save_sample(
        ...
        if valid_batches > 0:
            ...
            # Save checkpoint every 199 epochs
            if (epoch + 1) % 199 == 0:
