...
#
# 用法：
#   nohup bash exp_0226/watch_and_launch_0226a.sh > /tmp/watch_0226a.log 2>&1 &
...
# -------------------------------------------------------
while true; do
    LATEST=$(ls -dt "$BASE/exp_0226/runs/no_vq_e2e_epoch_"* 2>/dev/null | head -1 || true)
...
nohup "$CONDA_PYTHON" \
    "$BASE/exp_0226/train_enc_feat_align.py" \
    --mode epoch \
    --epochs 300 \
    --device "$DEVICE" \
