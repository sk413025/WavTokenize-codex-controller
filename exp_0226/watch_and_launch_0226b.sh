...
#
# 用法：
#   nohup bash exp_0226/watch_and_launch_0226b.sh > /tmp/watch_0226b.log 2>&1 &
...
# -------------------------------------------------------
while true; do
    LATEST=$(ls -dt "$BASE/exp_0226/runs/enc_feat_align_epoch_"* 2>/dev/null | head -1 || true)
...
# ENCODER_CKPT=$(ls -t "$BASE"/exp_0226/runs/enc_feat_align_epoch_*/best_model_val_total.pt 2>/dev/null | head -1 || true)
...
nohup "$CONDA_PYTHON" \
    "$BASE/exp_0226/train_enc_hf_mel.py" \
    --mode epoch \
    --epochs 300 \
    --device "$DEVICE" \
