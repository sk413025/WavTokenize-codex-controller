...
# ============================================================
# Losses (same as exp_0226a)
# ============================================================

class STFTLoss(nn.Module):
    """單一解析度 STFT Loss (SC + Log-Mag)。
    ...
    def forward(self, y_hat: torch.Tensor, y: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            (spectral_convergence_loss, log_magnitude_loss) 二元組。
        """
...
class MultiResolutionSTFTLoss(nn.Module):
    ...
    使用多組 FFT 設定分別計算 SC + Log-Mag Loss 再取平均。

    Args:
        fft_sizes: 各解析度 FFT 大小列表。
        hop_sizes: 各解析度 Hop 大小列表。
        win_sizes: 各解析度窗口大小列表。
    """
    ...
    def forward(self, y_hat: torch.Tensor, y: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            y_hat: 預測波形 (B, 1, T) 或 (B, T)。
            y: 目標波形 (B, 1, T) 或 (B, T)。

        Returns:
            (平均 SC Loss, 平均 Log-Mag Loss) 二元組。
        """
        if y_hat.dim() == 3:
...
def main():
    ...
    if args.output_dir:
        ...
        exp_dir = Path(f'exp_0304/runs/material_gen_{args.mode}_{timestamp}')
