...
class SimpleTransformer(nn.Module):
    def __init__(self, input_dim, hidden_dim, projection_dim, num_heads, num_layers):
        ...
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=input_dim, nhead=num_heads, batch_first=True),
    ...
    def forward(self, x):
        ...
        x = self.fc(x)  # 在時間維度上保持形狀
        x = x.transpose(1, 2)  # 轉回 [B, C, T]
        return x

class EnhancedFeatureExtractor(nn.Module):
    def __init__(self, config_path, model_path, byol_checkpoint_path=None, num_residual_blocks=2):
        ...
        # 4. BYOL Transformer
        hidden_dim = 128  # 根據您的train_byol.py設定調整
        projection_dim = 128
        num_heads = 4
        num_layers = 2
        self.byol_transformer = SimpleTransformer(encoder_dim, hidden_dim, projection_dim, num_heads, num_layers)
        
        # 5. 載入BYOL Transformer權重
        if byol_checkpoint_path:
            self.load_byol_transformer_weights(byol_checkpoint_path)
        
        # 6. 決定是否凍結 BYOL Transformer 權重
        self.freeze_byol_transformer = False  # 默認不凍結，可以在配置中設定
        if self.freeze_byol_transformer:
            for param in self.byol_transformer.parameters():
                param.requires_grad = False

        # 7. 初始化特徵增強層
        # Changed: Swap input and output channels in adapter_conv
        self.adapter_conv = nn.Conv1d(512, 256, kernel_size=1)
    ...
    def load_byol_transformer_weights(self, checkpoint_path):
        """載入 BYOL Transformer 權重"""
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        self.byol_transformer.load_state_dict(checkpoint, strict=False)
        print(f"Loaded BYOL Transformer weights from: {checkpoint_path}")
    ...
    def forward(self, x):
        ...
        # 使用 no_grad 進行編碼
        with torch.no_grad():
            features = self.encodec.encoder(x)  # [B, 512, T]
            # 增加初始特徵的振幅並正規化
            features = features * 3.0  # 提高初始振幅
            features = F.layer_norm(features.transpose(1, 2), [features.size(1)]).transpose(1, 2)
        
        # 將編碼器 features 餵給 Byol transformer 
        features = self.byol_transformer(features)  # [B, 512, 1]
        
        # 使用預訓練後的編碼表示作為特徵增強模塊的輸入
        features = self.adapter_conv(features)  # [B, 256, 1]
        ...
        # 確保最終輸出有足夠的振幅
        features = self.relu(features) * 2.5  # 增加最終特徵的振幅
        
        # 添加強制正規化步驟
        features = F.layer_norm(features.transpose(1, 2), [features.size(1)]).transpose(1, 2) * 3.0
...
def compute_optimized_loss(output, target, device):
    ...
    def quick_stft_loss(x, y, n_fft=2048):
        ...
        # 計算magnitude和phase損失
        mag_loss = torch.abs(torch.abs(x_stft) - torch.abs(y.stft))
        phase_loss = 1 - torch.cos(torch.angle(x_stft) - torch.angle(y.stft))
...
def compute_optimized_loss_v2(output, target, device, lambda_perceptual=0.1):
    ...
    def spectral_detail_loss(x, y, n_fft=2048):
        ...
        # 細節損失
        detail_loss = torch.abs(torch.abs(x_stft) - torch.abs(y.stft))
...
def compute_voice_focused_loss(output, target, device):
    ...
    def stft_loss(x, y, n_fft=2048):
        ...
        voice_weights = torch.ones_like(freqs)
        voice_mask = ((freqs >= 80) & (freqs <= 3400)).float()
        voice_weights = voice_weights + voice_mask * 2.0
        
        # 計算損失
        mag_loss = torch.abs(x_stft.abs() - y_stft.abs())  # 修改這裡
        phase_loss = 1 - torch.cos(x_stft.angle() - y_stft.angle())  # 修改這裡
...
def train_model(model, train_loader, optimizer, scheduler, device, save_dir, config):
    ...
    best_loss = float('inf')
    scaler = torch.cuda.amp.GradScaler()  # 添加混合精度訓練的 scaler
    ...
    for epoch in range(config['epochs']):
        ...
        for batch_idx, (input_wav, target_wav) in progress_bar:
            try:
                ...
                # 移動數據到設備並確保數據類型
                input_wav = input_wav.to(device, non_blocking=True, dtype=torch.float32)
                target_wav = target_wav.to(device, non_blocking=True, dtype=torch.float32)
