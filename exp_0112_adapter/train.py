"""
exp_0112_adapter: Exp J - 中層 Adapter 去噪訓練

核心設計:
- 在噪音敏感層 (L5-L8) 之前插入可訓練的 Adapter
- 只訓練 Adapter 參數，原始 WavTokenizer 完全凍結
- Adapter 專門學習去噪，容量受控避免過擬合

配置:
- Adapter 位置: L4 之後 (downsample 輸出)
- Adapter 類型: simple (bottleneck) 或 multihead
- Adapter hidden_dim: input_dim // 4 (預設)
- 可學習 scale 參數控制影響程度

執行:
    python exp_0112_adapter/train.py --exp_name exp_j_adapter
"""
...
from exp_0112_adapter.models import TeacherStudentAdapter
...
def train_epoch(model, dataloader, optimizer, loss_fn, device, epoch,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        metrics['adapter_scale'] += output.get('adapter_scale', 0)
        ...
        pbar.set_postfix({
            ...
            'scale': f"{output.get('adapter_scale', 0):.4f}"
        })
...
def plot_metrics(history, exp_dir):
    ...
    # Adapter Scale
    ax = axes[1, 2]
    ax.plot(history['adapter_scale'], color='purple')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Scale')
    ax.set_title('Adapter Scale Parameter')
    ax.grid(True)
    ...
    plt.suptitle('Exp J: Mid-Layer Adapter Denoising', fontsize=14)
    plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description='Exp 0112_adapter: 中層 Adapter 去噪 (Exp J)')

    # Experiment
    parser.add_argument('--exp_name', type=str, default='exp_j_adapter')
    parser.add_argument('--output_dir', type=str, default=None)

    # Adapter Config
    parser.add_argument('--adapter_position', type=int, default=4,
                        help='Adapter 插入位置 (L4 之後)')
    parser.add_argument('--adapter_type', type=str, default='simple',
                        choices=['simple', 'multihead'],
                        help='Adapter 類型')
    parser.add_argument('--adapter_hidden_dim', type=int, default=None,
                        help='Adapter hidden dim (None = input_dim // 4)')
    ...
    config['experiment_type'] = 'Exp J: Mid-Layer Adapter Denoising'

    with open(exp_dir / 'config.json', 'w') as f:
        json.dump(config, f, indent=2)

    print("=" * 60)
    print(f"Exp 0112_adapter: {config['experiment_type']}")
    print(f"Experiment: {args.exp_name}")
    print("=" * 60)
    print(f"\nAdapter Configuration:")
    print(f"  Position: After L{args.adapter_position} (before L5)")
    print(f"  Type: {args.adapter_type}")
    print(f"  Hidden dim: {args.adapter_hidden_dim or 'input_dim // 4'}")
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        history['adapter_scale'].append(train_metrics['adapter_scale'])
        ...
        print(f"\nTrain: Loss={train_metrics['total_loss']:.4f}, "
              ...
              f"Adapter Scale={train_metrics['adapter_scale']:.4f}")
        ...
        if val_metrics['masked_acc'] > best_val_acc:
            ...
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.get_adapter().state_dict(),
    ...
    print(f"Final Adapter Scale: {history['adapter_scale'][-1]:.4f}")
    print(f"\n結果保存於: {exp_dir}")
