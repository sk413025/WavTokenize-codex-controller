...
目的：
- 定期重置未使用的 codebook entries，防止 codebook collapse
- Short-run: 1000 steps
- 基於 exp_k v6 baseline 配置

方法：
    每 refresh_interval 步驟檢查 codebook usage
    重置使用次數 < threshold 的 codebook entries
...
執行：
    CUDA_VISIBLE_DEVICES=0 python exp_0128/phase2/codebook_refresh/train_codebook_refresh.py \\
        ...
        --usage_threshold 10 \\
        --steps 1000 \\
        --output_dir exp_0128/phase2/codebook_refresh/run_interval_100_thresh_10 \\
        --seed 42
"""
...
class CodebookUsageTracker:
    ...
    def update(self, codes):
        ...
        counts = torch.bincount(codes_flat, minlength=self.codebook_size)
        self.usage_count += counts

    def get_unused_codes(self, threshold):
        """獲取使用次數 < threshold 的 code indices"""
        return (self.usage_count < threshold).nonzero(as_tuple=True)[0]
    ...
    def refresh_codebook(self, model, threshold, step):
        ...
        Returns:
            num_refreshed: 重置的 codebook entry 數量
        """
        unused_codes = self.get_unused_codes(threshold)
        num_refreshed = len(unused_codes)

        if num_refreshed > 0:
            # Get the codebook from model
            # WavTokenizer VQ codebook: model.student.feature_extractor.encodec.quantizer.vq.layers[0]._codebook.embed
            vq_layer = model.student.feature_extractor.encodec.quantizer.vq.layers[0]
            codebook = vq_layer._codebook.embed  # [codebook_size, dim]

            with torch.no_grad():
                # Reinitialize unused codes with random vectors (same init as original)
                # Use normal distribution with same std as existing codebook
                ...
                codebook[unused_codes] = new_vectors

            # Reset usage count for refreshed codes
            ...
            self.refresh_history.append({
                ...
                'usage_stats': {
                    'min': self.usage_count.min().item(),
                    'max': self.usage_count.max().item(),
                    'mean': self.usage_count.float().mean().item(),
                    'num_unused': num_refreshed,
                }
            })
    ...
    def get_stats(self):
        ...
        return {
            ...
            'max': self.usage_count.max().item(),
            'mean': self.usage_count.float().mean().item(),
            'std': self.usage_count.float().std().item(),
            'num_used': (self.usage_count > 0).sum().item(),
            'num_unused': (self.usage_count == 0).sum().item(),
        }
...
def plot_loss_curves(loss_history, codebook_tracker, save_path, refresh_interval, usage_threshold):
    """繪製訓練 Loss 曲線和 codebook 使用統計"""
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(f'Exp 0128 Phase 2: Codebook Refresh (interval={refresh_interval}, threshold={usage_threshold})',
                 fontsize=14)
    ...
    ax = axes[1, 1]
    refresh_steps = [x['step'] for x in codebook_tracker.refresh_history]
    num_refreshed = [x['num_refreshed'] for x in codebook_tracker.refresh_history]
    num_unused = [x['usage_stats']['num_unused'] for x in codebook_tracker.refresh_history]

    if refresh_steps:
        ax.bar(refresh_steps, num_refreshed, width=refresh_interval*0.8, alpha=0.7, label='Num Refreshed')
        ax.plot(refresh_steps, num_unused, 'ro-', label='Num Unused', markersize=8)

    ax.set_title('Codebook Refresh Statistics')
    ax.set_xlabel('Step')
...
def main():
    ...
    parser.add_argument('--refresh_interval', type=int, required=True,
                        help='Refresh codebook every N steps (e.g., 50, 100, 200)')
    parser.add_argument('--usage_threshold', type=int, required=True,
                        help='Minimum usage count to keep a code (e.g., 5, 10, 20)')
    ...
    output_dir = Path(args.output_dir) / f"run_interval{args.refresh_interval}_thresh{args.usage_threshold}_{timestamp}"
    ...
    config['method'] = f'codebook_refresh_interval_{args.refresh_interval}_threshold_{args.usage_threshold}'
    ...
    # Create codebook usage tracker
    codebook_tracker = CodebookUsageTracker(codebook_size=2048)
    ...
    while step < args.steps:
        ...
        with autocast():
            ...
            codebook_tracker.update(output['student_codes'])
        ...
        pbar.set_postfix({
            ...
            'loss_inter': f'{loss_inter.item():.4f}',
        })
        pbar.update(1)

        step += 1

        # **NEW: Codebook Refresh**
        if step % args.refresh_interval == 0:
            num_refreshed = codebook_tracker.refresh_codebook(model, args.usage_threshold, step)
            stats = codebook_tracker.get_stats()
            print(f"\n  [Step {step}] Codebook Refresh:")
            print(f"    Refreshed {num_refreshed} codes (usage < {args.usage_threshold})")
        ...
        if step % args.save_checkpoint_every == 0 or step == args.steps:
            ...
            torch.save({
                ...
                'config': config,
                'codebook_usage': codebook_tracker.usage_count,
                'refresh_history': codebook_tracker.refresh_history,
            }, ckpt_path)
        ...
        if step % args.eval_interval == 0 or step == args.steps:
            ...
            with open(output_dir / 'codebook_refresh_history.json', 'w') as f:
                json.dump(codebook_tracker.refresh_history, f, indent=2)

            # Plot curves
            plot_loss_curves(loss_history, codebook_tracker, output_dir / 'training_curves.png',
                           args.refresh_interval, args.usage_threshold)
    ...
    final_stats = codebook_tracker.get_stats()
    print(f"\nFinal codebook usage stats:")
    print(f"  Used codes: {final_stats['num_used']}")
    print(f"  Unused codes: {final_stats['num_unused']}")
    print(f"  Mean usage: {final_stats['mean']:.1f}")
    print(f"  Total refreshes: {len(codebook_tracker.refresh_history)}")
    ...
    # Save final summary
    summary = {
        'experiment': 'exp_0128_phase2_codebook_refresh',
        ...
        'codebook_stats': final_stats,
        'total_refreshes': len(codebook_tracker.refresh_history),
        'success': (
