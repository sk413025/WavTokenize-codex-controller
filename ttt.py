...
class EnhancedFeatureExtractor(nn.Module):
    ...
    def forward(self, x, bandwidth_id=None):
        ...
        with torch.no_grad():
            try:
                # 檢查bandwidth_id形狀和類型
                print(f"Debug - bandwidth_id before encode: shape={bandwidth_id.shape}, dtype={bandwidth_id.dtype}, value={bandwidth_id}")
                ...
                if isinstance(bandwidth_id, torch.Tensor):
                    if bandwidth_id.dim() > 0 and bandwidth_id.shape[0] > 1:
                        ...
                        # 注意：由於feature_extractors.py中也會處理，這裡不需要調用item()
                        ...
                        print(f"Debug - converted bandwidth_id to item: {bandwidth_id_processed}, type={type(bandwidth_id_processed)}")
                
                input_features, discrete_code = self.wavtokenizer.encode_infer(x, bandwidth_id=bandwidth_id_processed)
...
def compute_hybrid_loss_with_content(output, target_wav, enhanced_features, target_features, 
    ...
    Args:
        ...
        intermediate_enhanced_features (torch.Tensor): 中間特徵（用於內容一致性）
...
# 簡化為與compute_hybrid_loss_with_content相同的實現，以便統一損失計算邏輯
def compute_hybrid_loss_with_tsne_flow(output, target_wav, enhanced_features, target_features, 
    ...
    在當前的設計中，此函數功能與compute_hybrid_loss_with_content相同，
    保留此函數是為了向後兼容已有的命令行參數和訓練流程。
...
def plot_learning_curves(epochs, train_losses, val_losses, feature_losses, voice_losses, lr_values, save_path, content_losses=None):
    """
    繪製訓練損失曲線，只顯示總損失、內容一致性損失和L2損失
    
    參數:
        epochs: 訓練輪次記錄
        train_losses: 總訓練損失記錄
        val_losses: 驗證損失記錄
        feature_losses: 特徵損失記錄 (L2損失)
        voice_losses: 聲音損失記錄 (不顯示)
        lr_values: 學習率記錄
        save_path: 保存路徑
        content_losses: 內容一致性損失記錄
    """
    ...
    # 總訓練損失曲線
    line1 = ax1.plot(epochs, train_losses, 'b-', linewidth=2, label='Total Loss')
    
    # L2損失曲線 (特徵損失)
    line2 = ax1.plot(epochs, feature_losses, 'g-', linewidth=2, label='L2 Loss')
    
    # 內容一致性損失曲線
    lines_content = []
    if content_losses is not None and len(content_losses) > 0:
        line_content = ax1.plot(epochs, content_losses, 'r-', linewidth=2, label='Content Consistency Loss')
    ...
    '''
    # 學習率曲線
    line4 = ax2.plot(epochs, lr_values, 'm--', linewidth=1.5, label='Learning Rate')
    ax2.set_ylabel('Learning Rate', color='m', fontsize=12)
    ax2.tick_params(axis='y', labelcolor='m')
    '''
    # 合併圖例
    lines = line1 + line2 + lines_content
    labels = [l.get_label() for l in lines]
    plt.legend(lines, labels, fontsize=11, loc='upper right')
    
    plt.title('訓練過程損失與學習率變化曲線', fontsize=14)
    plt.tight_layout()
    plt.savefig(save_path)
    plt.close()
...
def train_model(model, train_loader, optimizer, device, save_dir, config, num_epochs=100, scheduler=None, val_loader=None, use_content_loss=True):
    ...
    if (os.path.exists(best_model_path)):
        ...
        print(f"Loaded previous best loss: {best_loss}")
...
def train_model(model, train_loader, optimizer, device, save_dir, config, num_epochs=100, scheduler=None, val_loader=None, use_content_loss=True):
    ...
    if (os.path.exists(best_model_path)):
        ...
        if 'epochs_record' in checkpoint:
            ...
            # 嘗試恢復內容衰減因子記錄
            if 'content_decay_factors' in checkpoint:
                content_decay_factors = checkpoint['content_decay_factors']
                print(f"已恢復之前的內容衰減因子記錄，共 {len(content_decay_factors)} 個記錄點")
            else:                # 如果之前沒有內容衰減因子記錄，則創建相同長度的零記錄
                content_decay_factors = [0.0] * len(epochs_record)
                print("找不到之前的內容衰減因子記錄，使用零值初始化")
    else:
    ...
    if not ('epochs_record' in locals() and epochs_record):
        ...
        voice_losses_record = []
        lr_values_record = []
        content_losses_record = []  # 記錄內容一致性損失
        content_decay_factors = []  # 初始化內容衰減因子記錄列表
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (input_wav, target_wav, *extra_data) in progress_bar:
            ...
            # 獲取目標音頻的特徵
            with torch.no_grad():
                # 處理bandwidth_id，確保兼容性
                bandwidth_id_processed = bandwidth_id
                if isinstance(bandwidth_id, torch.Tensor) and bandwidth_id.dim() > 0 and bandwidth_id.shape[0] > 1:
                    print(f"目標特徵提取 - 批次bandwidth_id處理：使用第一個元素 {bandwidth_id[0]}")
                    
                # 修改為使用與input相同的方法提取target特徵
                target_features, target_discrete_code = model.feature_extractor.wavtokenizer.encode_infer(target_wav, bandwidth_id=bandwidth_id_processed)
            ...
            # 使用最後一層L2損失代替最終層的特徵損失
            if "last_layer_l2_loss" in loss_details:
                total_feature_loss += loss_details["last_layer_l2_loss"]
            else:
                total_feature_loss += loss_details["feature_loss"]
            total_voice_loss += loss_details["voice_loss"]
            
            # 改用與實際計算一致的損失累積方式
            if "penultimate_layer_content_loss" in loss_details:
                # 使用倒數第二層內容損失代替平均或最終層內容損失
                content_loss = loss_details.get("penultimate_layer_content_loss", 0.0)
                total_content_loss += content_loss
            elif "content_consistency_loss" in loss_details or "avg_layer_content_loss" in loss_details:
                # 向後兼容舊版本
        ...
        # 確保content_decay_factors已被初始化
        if 'content_decay_factors' not in locals() or content_decay_factors is None:
        ...
        if val_loader is not None:
            ...
            with torch.no_grad():
                for batch_data in val_loader:
                    ...
                    bandwidth_id_processed = bandwidth_id
                    if isinstance(bandwidth_id, torch.Tensor) and bandwidth_id.dim() > 0 and bandwidth_id.shape[0] > 1:
                        print(f"評估階段 - 批次bandwidth_id處理：使用第一個元素 {bandwidth_id[0]}")
                        
                    target_features, _ = model.feature_extractor.wavtokenizer.encode_infer(target_wav, bandwidth_id=bandwidth_id_processed)
        ...
        # 每150個 epoch 繪製一次學習曲線以及t-SNE可視化圖
        if (epoch + 1) % 150 == 0:            # 保存學習曲線
            curve_path = os.path.join(save_dir, f'learning_curve_epoch_{epoch+1}.png')
            plot_learning_curves(
                epochs_record, train_losses_record, val_losses_record, 
                feature_losses_record, voice_losses_record, lr_values_record, 
            ...
            try:
                ...
                with torch.no_grad():
                    for batch_idx, batch_data in enumerate(train_loader):
                        ...
                        bandwidth_id_processed = bandwidth_id
                        if isinstance(bandwidth_id, torch.Tensor) and bandwidth_id.dim() > 0 and bandwidth_id.shape[0] > 1:
                            print(f"t-SNE可視化 - 批次bandwidth_id處理：使用第一個元素 {bandwidth_id[0]}")
                            
                        batch_target_features, _ = model.feature_extractor.wavtokenizer.encode_infer(target_wav, bandwidth_id=bandwidth_id_processed)
                ...
                # 處理特徵列表 - 保留完整的批次結構，而不是合併所有批次
                if all_enhanced_features and all_target_features:
                    # 找出最小的特徵時間維度，確保每個批次內的所有樣本有相同的時間長度
                    min_time_dims = []
                    for enh_feat, tgt_feat in zip(all_enhanced_features, all_target_features):
                        min_dim = min(enh_feat.size(-1), tgt_feat.size(-1))
                        min_time_dims.append(min_dim)
                    
                    # 處理每個批次，保留批次結構（每句話為一個點）
                    for i in range(len(all_enhanced_features)):
                        # 截斷到相同的時間長度
                        all_enhanced_features[i] = all_enhanced_features[i][..., :min_time_dims[i]]
                        all_target_features[i] = all_target_features[i][..., :min_time_dims[i]]
                    
                    # 計算總樣本數
                    total_samples = sum(feat.size(0) for feat in all_enhanced_features)
                    print(f"為t-SNE收集了 {len(all_enhanced_features)} 個批次，共 {total_samples} 個樣本")
                    
                    # 生成增強型t-SNE可視化（每句話為一個點）
                    plot_tsne_visualization(
                        enhanced_features=all_enhanced_features,
                        target_features=all_target_features,
                        save_path=tsne_save_path
                    )
                    
                    # 生成額外的詳細t-SNE視覺化（保存到子目錄）
                    detailed_tsne_dir = os.path.join(tsne_dir, "detailed")
                    os.makedirs(detailed_tsne_dir, exist_ok=True)
                    detailed_save_path = os.path.join(detailed_tsne_dir, f'tsne_detailed_epoch_{epoch+1}.png')
                    
                    # 合併批次以生成備用視覺化
                    min_global_time = min(min_time_dims)
                    all_enh_uniform = [feat[..., :min_global_time] for feat in all_enhanced_features]
                    all_tgt_uniform = [feat[..., :min_global_time] for feat in all_target_features]
                    
                    # 合併所有批次
                    enhanced_features_for_tsne = torch.cat(all_enh_uniform, dim=0)
                    target_features_for_tsne = torch.cat(all_tgt_uniform, dim=0)
                    
                    # 保存備用的t-SNE視覺化（使用原始時間維度，用於比較）
                    experiment_date = datetime.datetime.now().strftime('%Y%m%d')
                    with open(os.path.join(tsne_dir, f'tsne_features_epoch_{epoch+1}_{experiment_date}.pt'), 'wb') as f:
                        torch.save({
                            'enhanced_features': enhanced_features_for_tsne.detach().cpu(),
                            'target_features': target_features_for_tsne.detach().cpu(),
                            'epoch': epoch + 1
                        }, f)
                    print(f"✅ Epoch {epoch+1} 的t-SNE特徵數據已保存")
        ...
        # 每100個 epoch 以及最後一輪時保存 t-SNE 視覺化
        if (epoch + 1) % 100 == 0 or epoch == num_epochs - 1:
            ...
            with torch.no_grad():
                ...
                epoch_offset = (epoch // 300) % 10  # 每10次採樣循環一次
    ...
    # 更新實驗報告
    update_experiment_report(config, save_dir, train_losses_record[-1], content_losses_record[-1], feature_losses_record[-1], final_curve_path)
...
def main():
    ...
    parser.add_argument("--first_two_blocks_only", action="store_true", help="使用特殊損失分配策略：倒數第二層專注於內容一致性損失，第一層和其他中間層不計算損失（自由學習），最後一層專注於L2特徵損失")
    ...
    if args.first_two_blocks_only:
        print("\n✅ 啟用了特殊損失分配策略：倒數第二層專注於內容一致性損失，第一層和其他中間層不計算損失（自由學習），最後一層專注於L2特徵損失")
    # 從環境變數獲取實驗ID
    exp_id = os.environ.get('TTT_EXPERIMENT_ID', datetime.datetime.now().strftime('%Y%m%d%H%M'))
    ...
    # 使用實驗ID作為目錄名
    output_dir = os.path.join(output_base_dir, f"output4_{exp_id}")
    ...
    # 從環境變數獲取批次大小和工作線程數，如果未設置則使用默認值
    batch_size = int(os.environ.get('TTT_BATCH_SIZE', 4))  # 默認值為4
    num_workers = int(os.environ.get('TTT_NUM_WORKERS', 2))  # 默認值為2
    
    print(f"\n✅ 從環境變數獲取配置:")
    print(f"   - 批次大小 (TTT_BATCH_SIZE): {batch_size}")
    print(f"   - 工作線程數 (TTT_NUM_WORKERS): {num_workers}")
    ...
    config = {
        ...
        'save_dir': output_dir,
        'epochs': 300,              # 設定訓練輪數
        'batch_size': batch_size,    # 使用環境變數中的值
        ...
        'min_lr': 1e-6,
        'feature_scale': 1.5,
        'num_workers': num_workers,  # 使用環境變數中的值
        'pin_memory': True,          
        'prefetch_factor': 2,       
        ...
        'min_content_samples': 3,            # 每批次中相同content_id的最小樣本數
...
def compute_content_consistency_loss(intermediate_features, content_ids, device):
    """
    計算內容一致性損失 - 改進版，參考 en_train.py
    
    對於批次中具有相同content_id的樣本，計算它們的中間特徵之間的餘弦相似度。
    同時考慮特徵的方向（餘弦相似度）和大小（分布差異），更好地保留語句結構。
    ...
    for content_id in unique_content_ids:
        ...
        # 【改進 1】: 直接計算原始特徵與均值的餘弦相似度（參考 en_train.py）
        # 這保留了特徵的大小信息，有助於更好地保持語句結構
        direct_cos_sim = F.cosine_similarity(group_features, mean_feature, dim=1)
        direct_distances = 1.0 - direct_cos_sim
        
        # 【改進 2】: 也計算特徵的L2距離，捕捉幅值/分布差異
        l2_distances = torch.sqrt(torch.sum((group_features - mean_feature) ** 2, dim=1))
        # 標準化L2距離避免尺度問題
        l2_distances = l2_distances / (torch.mean(l2_distances) + 1e-8)
        
        # 【改進 3】: 綜合考慮方向相似度和分布差異
        # alpha 參數控制分布差異的權重
        alpha = 0.3
        combined_distances = direct_distances + alpha * l2_distances
        
        # 累加這組樣本的平均距離到損失值
        group_loss = torch.mean(combined_distances)
    ...
    # 如果是第一次計算或隨機採樣，輸出損失組件以便調試
    if torch.rand(1).item() < 0.01:  # 1%機率輸出日誌，避免太多日誌
        try:
            print(f"\n【改進內容損失】樣本數: {batch_size}, 有效組: {valid_groups}")
            if 'direct_distances' in locals() and 'l2_distances' in locals():
                print(f"  - 方向損失: {torch.mean(direct_distances).item():.4f}")
                print(f"  - 分布損失: {torch.mean(l2_distances).item():.4f}")
                print(f"  - 合併損失: {torch.mean(combined_distances).item():.4f}")
        except:
            pass
...
def compute_layered_hybrid_loss(output, target_wav, enhanced_features, target_features, 
    ...
    """
    更加簡化的分層損失函數 - 總損失僅由倒數第二層內容一致性損失和最後一層L2特徵損失組成。
    倒數第二層專注於內容一致性損失，第一層和其他中間層不計算損失（自由學習），最後層專注於L2特徵損失。
    不使用層級重要性權重或最終層額外權重，使損失計算更加直觀和簡單。
    ...
    if target_features.dim() == 2:
        target_features = target_features.unsqueeze(0)
    # 計算最終層的L2損失
    final_l2_loss = compute_feature_loss(enhanced_features, target_features, device)
    
    # 計算內容一致性損失（改為第二層，索引1）
    if len(intermediate_features_list) > 1:
        final_content_loss = compute_content_consistency_loss(intermediate_features_list[1], content_ids, device)
    else:
        final_content_loss = torch.tensor(0.0, device=device)
    ...
    for i, layer_features in enumerate(intermediate_features_list):
        # 計算這一層的內容一致性損失
        layer_content_loss = compute_content_consistency_loss(layer_features, content_ids, device)
        content_losses.append(layer_content_loss)
        
        # 對於比較深的層，可以計算與目標特徵的L2損失
        # 先檢查特徵維度是否匹配，如不匹配則進行維度轉換
        if layer_features.shape[1] != target_features.shape[1]:
            # 獲取維度信息
            layer_dim = layer_features.shape[1]
            target_dim = target_features.shape[1]
            
            # 創建適合的投影層
            proj_name = f"dim_proj_{layer_dim}_{target_dim}"
            if not hasattr(compute_layered_hybrid_loss, proj_name):
                print(f"創建特徵維度投影層: {layer_dim} -> {target_dim}")
                setattr(compute_layered_hybrid_loss, proj_name, 
                       nn.Conv1d(layer_dim, target_dim, kernel_size=1).to(device))
                # 初始化投影層
                proj = getattr(compute_layered_hybrid_loss, proj_name)
                nn.init.xavier_uniform_(proj.weight)
                nn.init.zeros_(proj.bias)
              # 使用投影層將層特徵維度調整為與目標特徵一致
            proj = getattr(compute_layered_hybrid_loss, proj_name)
            projected_features = proj(layer_features)
            layer_l2_loss = compute_feature_loss(projected_features, target_features, device)
        else:
            # 維度相同，直接計算損失
            layer_l2_loss = compute_feature_loss(layer_features, target_features, device)
        
        l2_losses.append(layer_l2_loss)
        
        # 根據層級調整權重：前層內容權重高，後層L2權重高
        # 檢查是否啟用了 first_two_blocks_only 模式（使用全局參數）
        # 獲取全局的 args 變數
        import __main__
        args = getattr(__main__, 'args', None)
        first_two_blocks_only = args.first_two_blocks_only if args else '--first_two_blocks_only' in sys.argv
        
        # 原始設定：保留 first_two_blocks_only 模式的核心思想
        if first_two_blocks_only:
            # 修改：倒數第二層專注內容，其他中間層不計算損失，最後層專注音質
            if i == 0:
                # 第一層：完全不計算損失（自由學習）
                raw_content_weight = 0.0
                raw_l2_weight = 0.0
                print(f"\rLayer {i+1}/{num_layers}: 不計算損失（自由學習）", end='')
            elif i == num_layers-2:
                # 倒數第二層：100% 內容一致性損失
                raw_content_weight = 1.0
                raw_l2_weight = 0.0
                print(f"\rLayer {i+1}/{num_layers}: 內容一致性損失", end='')
            elif i == num_layers-1:
                # 最後層：100% L2特徵損失
                raw_content_weight = 0.0
                raw_l2_weight = 1.0
                print(f"\rLayer {i+1}/{num_layers}: L2 特徵損失", end='')
            else:
                # 其他中間層：完全不計算損失（跳過中間層損失計算）
                raw_content_weight = 0.0
                raw_l2_weight = 0.0
                print(f"\rLayer {i+1}/{num_layers}: 不計算損失（自由學習）", end='')
        else:
            # 標準模式下，漸進式權重轉變
            raw_content_weight = 1.0 - (i / max(num_layers-1, 1))  # 從1.0逐漸降至0.0
            raw_l2_weight = i / max(num_layers-1, 1)              # 從0.0逐漸升至1.0
        
        # 降低內容一致性權重：使用動態衰減因子 - 隨訓練進行逐漸降低內容一致性損失的權重，提高特徵對齊的權重
        # 初始內容衰減因子（訓練初期）與結束衰減因子（訓練後期）
        # 【修改：關閉衰減因子，固定為0】- EXP20250630_01
        initial_content_decay = 0.0  # 訓練初期，內容權重也為0（完全關閉內容一致性損失）
        final_content_decay = 0.0    # 訓練後期，內容權重降至0（完全不考慮內容損失）
        
        # 直接設定衰減因子為0，完全關閉內容一致性損失
        content_decay_factor = 0.0
        
        # 打印當前epoch的衰減因子（調試用）
        # print(f"Epoch {current_epoch}/{total_epochs} - 內容衰減因子: {content_decay_factor:.3f}")
          # 如果使用first_two_blocks_only模式，忽略content_decay_factor的影響
        if first_two_blocks_only:
            # 倒數第二層: 100% 內容一致性損失，0% L2損失
            # 中間層: 完全不計算損失，自由學習
            # 最後層: 0% 內容一致性損失，100% L2損失
            # 這裡直接使用原始權重設定，不受衰減因子影響
            content_weight = raw_content_weight
            l2_weight = raw_l2_weight
        else:
            # 標準模式: 應用動態衰減因子
            content_weight = raw_content_weight * content_decay_factor
            l2_weight = raw_l2_weight + (raw_content_weight * (1.0 - content_decay_factor))
        
        # 確保權重總和為1（當權重不全為0時）
        total_weight = content_weight + l2_weight
        ...
        if total_weight > 0:
            ...
            #     print(f"第{i+1}層：內容權重={content_weight:.2f}, L2權重={l2_weight:.2f}")
            
            # 計算該層的加權損失
            layer_loss = content_weight * layer_content_loss + l2_weight * layer_l2_loss
            weighted_losses.append(layer_loss)
        # 如果總權重為0，跳過這一層的損失計算
            
            # 簡化後只考慮倒數第二層內容損失和最後一層L2損失
            # 不添加層級重要性權重
            if i == num_layers-2 or i == num_layers-1:  # 只有倒數第二層和最後一層參與總損失
                total_loss = total_loss + layer_loss
        else:
            # 中間層不計算損失，添加0損失到列表以保持索引一致性
            weighted_losses.append(torch.tensor(0.0, device=device, requires_grad=False))
    
    # 移除最終層額外加權，讓總損失純粹是倒數第二層內容損失加最後一層L2損失
    # 不添加額外的final_l2_weight權重
    
    # 計算平均損失供記錄（排除0權重的中間層）
    non_zero_content_losses = [loss for i, loss in enumerate(content_losses) if i == len(content_losses)-2 or i == len(content_losses)-1]
    non_zero_l2_losses = [loss for i, loss in enumerate(l2_losses) if i == len(content_losses)-2 or i == len(l2_losses)-1]
    non_zero_weighted_losses = [loss for loss in weighted_losses if loss.item() > 0]
    
    avg_content_loss = sum(non_zero_content_losses) / len(non_zero_content_losses) if non_zero_content_losses else torch.tensor(0.0)
    avg_l2_loss = sum(non_zero_l2_losses) / len(non_zero_l2_losses) if non_zero_l2_losses else torch.tensor(0.0)
    
    # 添加更詳細的日誌信息
    if first_two_blocks_only and random.random() < 0.01:  # 隨機輸出1%的樣本以減少日誌量
        penultimate_layer_content_loss = content_losses[-2] if len(content_losses) > 1 else 0
        last_layer_l2_loss = l2_losses[-1] if len(l2_losses) > 0 else 0
        print(f"\n層數：{num_layers}，啟用損失層：倒數第二層(第{num_layers-1}層，內容一致性)和最後一層(第{num_layers}層，L2)")
        print(f"倒數第二層內容損失: {penultimate_layer_content_loss:.4f}, 第{num_layers}層L2損失: {last_layer_l2_loss:.4f}")
        print(f"總損失 = 倒數第二層內容損失 + 第{num_layers}層L2損失 = {(penultimate_layer_content_loss + last_layer_l2_loss):.4f}")
    
    avg_weighted_loss = sum(non_zero_weighted_losses) / len(non_zero_weighted_losses) if non_zero_weighted_losses else torch.tensor(0.0, device=device)
    
    # 新增直觀展示簡化計算的損失詳情
    # 內容一致性損失改為第二層（block2）
    penultimate_layer_content_loss = content_losses[1] if len(content_losses) > 1 else torch.tensor(0.0, device=device)
    last_layer_l2_loss = l2_losses[-1] if len(l2_losses) > 0 else torch.tensor(0.0, device=device)
    
    return total_loss, {
        'feature_loss': final_l2_loss.item(),
        'content_consistency_loss': final_content_loss.item(),
        'penultimate_layer_content_loss': penultimate_layer_content_loss.item(),  # 倒數第二層內容損失
        'last_layer_l2_loss': last_layer_l2_loss.item(),              # 最後層L2損失
        'avg_layer_content_loss': avg_content_loss.item(),            # 保留原始平均值以便向後兼容
        'avg_layer_l2_loss': avg_l2_loss.item(),                      # 保留原始平均值以便向後兼容
        'content_decay_factor': content_decay_factor,
        'avg_weighted_layer_loss': avg_weighted_loss.item(),
        'voice_loss': 0.0,  # 保持與現有API相容
        'total_loss': total_loss.item(),
        'simple_sum': (penultimate_layer_content_loss + last_layer_l2_loss).item()  # 直接相加的值，用於驗證
    }
...
def update_experiment_report(config, save_dir, final_total_loss, final_content_loss, final_l2_loss, curve_path):
    """
    更新實驗報告文件 (REPORT.md)，記錄本次實驗的結果
    
    Args:
        config: 配置参数字典
        save_dir (str): 保存目錄路徑
        final_total_loss (float): 最終的總損失值
        final_content_loss (float): 最終的內容一致性損失值 
        final_l2_loss (float): 最終的L2損失值
        curve_path (str): 學習曲線圖的路徑
    """
    from datetime import datetime
    
    report_path = "/home/sbplab/ruizi/WavTokenize/REPORT.md"
    current_date = datetime.now().strftime("%Y-%m-%d")
    experiment_id = f"EXP{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    
    # 提取實驗目錄名稱作為簡短描述
    exp_dir_name = os.path.basename(save_dir)
    
    # 構建新的實驗報告條目
    experiment_entry = f"""
## {current_date} 分層損失策略實驗 ({experiment_id})
- **實驗配置**: 
  - 倒數第二層專注內容損失
  - 中間層不計算損失（自由學習）
  - 最後層專注L2損失
...
- **保存目錄**: `{save_dir}`
- **最終損失值**:
  - 總損失: {final_total_loss:.6f}
  - 內容一致性損失: {final_content_loss:.6f}
  - L2損失: {final_l2_loss:.6f}
- **學習曲線**: ![學習曲線]({os.path.relpath(curve_path, "/home/sbplab/ruizi/WavTokenize")})
...
"""
    ...
    # 添加主要參數
    important_args = [
        "first_two_blocks_only", "use_layered_loss", "tsne_flow_with_content", 
        "content_consistency_weight", "l2_weight", "batch_size", "lr", "num_epochs"
    ]
    ...
    if config is not None:
        for arg in important_args:
            if arg in config:
                value = config[arg]
                experiment_entry += f"{arg}: {value}\n"
    else:
        # 如果 config 為 None，則從命令行參數或系統環境中檢測
        experiment_entry += "# 參數從命令行獲取，無法顯示具體值\n"
        if '--first_two_blocks_only' in sys.argv:
            experiment_entry += "first_two_blocks_only: True\n"
        if '--use_layered_loss' in sys.argv:
            experiment_entry += "use_layered_loss: True\n"
        if '--tsne_flow_with_content' in sys.argv:
            experiment_entry += "tsne_flow_with_content: True\n"
    ...
    try:
        ...
        report_content = "# 實驗記錄報告\n\n"
    
    # 在第一個標題後插入新實驗
    if "# 實驗記錄報告" in report_content:
        parts = report_content.split("# 實驗記錄報告", 1)
        report_content = "# 實驗記錄報告" + experiment_entry + parts[1]
    else:
        report_content = "# 實驗記錄報告\n" + experiment_entry + report_content
    
    # 寫入更新後的報告
    with open(report_path, 'w') as f:
        f.write(report_content)
    
    print(f"\n✅ 實驗結果已更新至 {report_path}")
    print(f"   實驗ID: {experiment_id}")
    print(f"   保存目錄: {save_dir}")



def compute_stft_loss(x, y, n_fft=2048, voice_weights=None):
    """
    計算STFT域的損失（幅度+相位），用於提升音質
    
    Args:
        x (torch.Tensor): 輸入波形
        y (torch.Tensor): 目標波形
        n_fft (int): FFT窗口大小
        voice_weights (torch.Tensor): 語音部分加權（可選）
        
    Returns:
        torch.Tensor: STFT損失
    """
    # 計算STFT
    x_stft = torch.stft(x.squeeze(1), n_fft, hop_length=n_fft//4, return_complex=True)
    y_stft = torch.stft(y.squeeze(1), n_fft, hop_length=n_fft//4, return_complex=True)
    ...
    if voice_weights is not None:
        ...
        weighted_loss = mag_loss + 0.3 * phase_loss
        
    return torch.mean(weighted_loss)


def compute_enhanced_stft_loss(output, target, device):
    """
    計算多分辨率STFT損失，用於增強音質
    
    Args:
        output (torch.Tensor): 輸出波形
        target (torch.Tensor): 目標波形
        device (torch.device): 計算設備
        
    Returns:
        torch.Tensor: 加權STFT損失
    """
    # 多分辨率STFT損失
    stft_loss_total = (
        compute_stft_loss(output, target, n_fft=2048) +  # 長時窗口，捕捉低頻特性
        compute_stft_loss(output, target, n_fft=1024) +  # 中等窗口
        compute_stft_loss(output, target, n_fft=512) +   # 短時窗口，捕捉高頻細節
        compute_stft_loss(output, target, n_fft=256)     # 超短窗口，捕捉瞬態和細節
    ) / 4.0
    
    return stft_loss_total


def compute_layered_hybrid_loss_with_final_stft(
        output, target_wav, enhanced_features, target_features, 
        intermediate_features_list, content_ids, device, current_epoch=0, total_epochs=100,
        l2_weight=0.6, stft_weight=0.4):
    """
    分層混合損失函數 - 最後一層增加STFT損失以提升音質
    ...
    Args:
        ...
        current_epoch (int): 當前訓練的epoch
        total_epochs (int): 總訓練epochs數
        l2_weight (float): L2損失權重
        stft_weight (float): STFT損失權重
    ...
    # 計算STFT損失（用於提升音質）
    final_stft_loss = compute_enhanced_stft_loss(output, target_wav, device)
    
    # 計算內容一致性損失 (通常使用最終層的中間特徵)
    final_content_loss = compute_content_consistency_loss(intermediate_features_list[-1], content_ids, device)
    ...
    for i, layer_features in enumerate(intermediate_features_list):
        ...
        if layer_features.shape[1] != target_features.shape[1]:
            ...
            # 創建適合的投影層
            proj_name = f"dim_proj_{layer_dim}_{target_dim}"
            if not hasattr(compute_layered_hybrid_loss_with_final_stft, proj_name):
                print(f"創建特徵維度投影層: {layer_dim} -> {target_dim}")
                setattr(compute_layered_hybrid_loss_with_final_stft, proj_name, 
                       nn.Conv1d(layer_dim, target_dim, kernel_size=1).to(device))
                # 初始化投影層
                proj = getattr(compute_layered_hybrid_loss_with_final_stft, proj_name)
                nn.init.xavier_uniform_(proj.weight)
                nn.init.zeros_(proj.bias)
        ...
        import __main__
        args = getattr(__main__, 'args', None)
        first_two_blocks_only = args.first_two_blocks_only if hasattr(args, 'first_two_blocks_only') else '--first_two_blocks_only' in sys.argv
        
        # 原始權重計算
        if first_two_blocks_only:
            # 只對倒數第二層有內容一致性損失，其餘層內容損失權重為0，L2損失權重為1
            raw_content_weight = 1.0 if i == 0 else 0.0
            raw_l2_weight = 0.0 if i == 0 else 1.0
            if i == 0 or i == num_layers-1:
                print(f"\rLayer {i+1}/{num_layers}: {'內容一致性損失' if i == 0 else 'L2 特徵損失'}", end='')
        else:
            # 標準模式下，漸進式權重轉變
            raw_content_weight = 1.0 - (i / max(num_layers-1, 1))  # 從1.0逐漸降至0.0
            raw_l2_weight = i / max(num_layers-1, 1)              # 從0.0逐漸升至1.0
        
        # 內容一致性損失衰減因子固定為0（完全關閉內容一致性損失）
        content_decay_factor = 0.0
        ...
        # 將每層損失添加到總損失，給更深的層更高的權重
        layer_importance = (i + 1) / num_layers
        total_loss = total_loss + layer_importance * layer_loss
    
    # 加入最終層的混合損失（L2特徵損失 + STFT損失）- 主要修改點
    final_layer_loss = l2_weight * final_l2_loss + stft_weight * final_stft_loss
    total_loss = total_loss + final_layer_loss
    
    # 計算平均損失供記錄
    avg_content_loss = sum(content_losses) / len(content_losses) if content_losses else torch.tensor(0.0)
    avg_l2_loss = sum(l2_losses) / len(l2_losses) if l2_losses else torch.tensor(0.0)
    avg_weighted_loss = sum(weighted_losses) / len(weighted_losses) if weighted_losses else torch.tensor(0.0)
    
    # 返回損失和詳細信息
    return total_loss, {
        'feature_loss': final_l2_loss.item(),
        'stft_loss': final_stft_loss.item(),
        'content_consistency_loss': final_content_loss.item(),
        'avg_layer_content_loss': avg_content_loss.item(),
        'avg_layer_l2_loss': avg_l2_loss.item(),
        'content_decay_factor': content_decay_factor,
        'avg_weighted_layer_loss': avg_weighted_loss.item(),
        'voice_loss': final_stft_loss.item(),  # 保持與現有API相容
        'total_loss': total_loss.item()
    }


def compute_feature_drift(enhanced_features, target_features):
    """
    計算特徵漂移，使用L2距離量化增強特徵與目標特徵的偏移程度
    ...
    # 計算L2距離
    l2_dist = torch.sqrt(torch.sum((enhanced_features - target_features) ** 2, dim=1))
...
# 最後一層混合損失模式: python ttt.py --use_mixed_final_loss --l2_weight 0.6 --stft_weight 0.4
# 損失計算邏輯說明：
#
...
#    - 實現函數：compute_layered_hybrid_loss
#
# 3. --use_mixed_final_loss：
#    - 最後一層使用混合損失 (L2特徵損失 + STFT損失)
#    - 實現函數：compute_layered_hybrid_loss_with_final_stft
#
# 3. --tsne_flow_with_L2：
