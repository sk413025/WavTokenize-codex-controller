"""
exp_0206 V2: Long-Term RVQ Training (300 Epochs) — Fixed Intermediate Weight

V2 變更（相對於 V1）:
- intermediate_weight 固定為 0.03（V1: 0.5 → warmdown to 0.25）
  原因: V1 intermediate loss 佔 total loss 94%，壓制 quant loss 梯度
- 移除 warmdown 機制（intermediate_weight_min, warmdown_epochs）
  原因: warmdown 是補償過高 intermediate_weight 的權宜之計，直接修正權重更本質
- 新增 NaN 保護（跳過 NaN batch，連續 NaN 時警告）
  原因: V1 在 epoch 177, 188 出現 NaN（L4 cosine similarity zero-norm）

架構不變:
1. TeacherStudentRVQ (LoRA + 4-layer RVQ, EMA codebook, dead-code reset)
2. 損失: L_quant(post-quant) + L_inter(fixed 0.03) + L_commit
3. Curriculum learning (noise 0.3→0.85 over 200 epochs)
4. Phase 3-2 P1/P2/P3 驗收

執行:
    bash exp_0206/run_v2.sh [GPU_ID]
"""
...
def train_epoch(model, dataloader, optimizer, inter_loss_fn, device, epoch,
    ...
    # V2: 固定中間層權重（不再使用 warmdown）
    intermediate_weight = config['intermediate_weight']

    # NaN 保護計數器
    nan_count = 0
    max_nan_per_epoch = 10  # 超過此數量則警告
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        # NaN 保護: 跳過產生 NaN 的 batch
        if torch.isnan(total_loss) or torch.isinf(total_loss):
            nan_count += 1
            metrics['nan_batches'] = nan_count
            print(f"  ⚠️ NaN/Inf detected at batch {batch_idx}, skipping (count: {nan_count})")
            optimizer.zero_grad()  # 清除可能已累積的梯度
            if nan_count >= max_nan_per_epoch:
                print(f"  🚨 Too many NaN batches ({nan_count}), consider checking data/model")
            continue
    ...
    if nan_count > 0:
        print(f"  📊 Epoch {epoch}: {nan_count} NaN batches skipped out of {n_batches + nan_count} total")
...
def main():
    ...
    # Intermediate supervision (V2: fixed weight, no warmdown)
    parser.add_argument('--intermediate_weight', type=float, default=0.03, help='中間層監督權重（固定，V2 預設 0.03）')
    ...
    exp_dir = Path(f'exp_0206/runs/{args.exp_name}_{timestamp}')
    ...
    print("=" * 70)
    print("exp_0206 V2: Long-Term RVQ Training (Fixed Intermediate Weight)")
    print("=" * 70)
    ...
    print(f"Loss: λ_quant={args.lambda_quant}, λ_pre={args.lambda_pre}, "
          f"β_commit={args.beta_commit}, inter={args.intermediate_weight} (fixed, no warmdown)")
    print(f"Curriculum: {args.curriculum_start}→{args.curriculum_end} over {args.curriculum_epochs} epochs")
    print(f"NaN protection: enabled (skip NaN batches, max 10/epoch warning)")
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        nan_info = f" NaN={train_metrics.get('nan_batches', 0)}" if train_metrics.get('nan_batches', 0) > 0 else ""
        print(f"  P2={val_metrics['p2_pass']} P3={val_metrics['p3_pass']} "
              f"LR={current_lr:.2e} IW={train_metrics['intermediate_weight']:.3f}{nan_info}")
        print(f"{'='*70}")
