...
基於 Exp55 (最佳 baseline, 0.91% Val Acc)，進一步擴大模型容量：
- LoRA rank: 256 -> 384 或 512
- Gradient Accumulation: 2 (回到 Exp55 設定)
- Epochs: 500 (增加訓練時間)
...
def train_epoch(model, dataloader, optimizer, loss_fn, device, epoch,
    ...
    for key in ['total_loss', 'feature_loss', 'triplet_loss', 'cosine_loss',
                'cos_sim_mean', 'masked_acc', 'unmasked_acc', 'distance_loss']:
...
def validate(model, dataloader, loss_fn, device, distance_matrix,
    ...
    for key in ['total_loss', 'feature_loss', 'triplet_loss', 'cosine_loss',
                'cos_sim_mean', 'masked_acc', 'distance_loss']:
        metrics[key] /= n_batches
...
def main():
    parser = argparse.ArgumentParser(description='Exp62: Capacity Expansion')

    # Experiment
    parser.add_argument('--exp_name', type=str, default='exp62_capacity')
    parser.add_argument('--output_dir', type=str, default=None)

    # LoRA - 擴大容量
    parser.add_argument('--lora_rank', type=int, default=384)  # 從 256 增加
    parser.add_argument('--lora_alpha', type=int, default=768)  # 保持 2x rank
    parser.add_argument('--lora_dropout', type=float, default=0.25)  # 略微增加
    parser.add_argument('--lora_layers', type=str, default='all_18')

    # Loss weights (與 Exp55 相同)
    ...
    # Training - 回到 Exp55 設定
    parser.add_argument('--lr', type=float, default=1e-4)  # Exp55 的 lr
    parser.add_argument('--weight_decay', type=float, default=0.05)
    parser.add_argument('--batch_size', type=int, default=8)
    parser.add_argument('--num_epochs', type=int, default=500)  # 增加
    ...
    # Gradient Accumulation - 回到 Exp55 設定
    ...
    print("=" * 60)
    print(f"Exp62: Capacity Expansion")
    print(f"Experiment: {args.exp_name}")
    print("=" * 60)
    print(f"LoRA Config:")
    print(f"  Rank: {args.lora_rank} (increased from 256)")
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        print(f"       Distance={val_metrics['distance_loss']:.4f}, LR={current_lr:.2e}")
