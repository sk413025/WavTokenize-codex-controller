...
基於 exp_1231_feature 分析結論設計：

問題診斷：
- 噪音主要影響 mid-level (L5-L6)，深層 (L13-L15) 對噪音魯棒
- 但 LoRA 訓練後深層變化最大 (1.73)，淺層變化最小 (1.43)
- 這與降噪任務需求相反，可能導致深層「硬記」而非學習去噪

實驗策略 (Exp B)：
- 只對淺/中層 (L0-L8) 加 LoRA，共 9 層
- 完全凍結深層 (L9-L17)
- 驗證假設：深層過度變化是 Train/Val gap 的原因

預期：
- 如果 Val acc 提升 → 確認深層變化是問題
- 如果 Val acc 下降 → 需要 Exp A (深層低 rank) 而非完全凍結
"""
...
def main():
    parser = argparse.ArgumentParser(description='Exp 1231_1: 凍結深層實驗 (Exp B)')

    # Experiment
    parser.add_argument('--exp_name', type=str, default='exp_shallow_lora')
    ...
    parser.add_argument('--lora_layers', type=str, default='shallow_mid',
                        choices=list(LORA_LAYER_PRESETS.keys()),
                        help='Layer preset: shallow_mid (L0-L8), noise_sensitive (L5-L8), all_18, deep_only')

    # Loss weights (與 Exp64 相同)
    ...
    # Curriculum Learning (與 Exp64 相同)
    ...
    config['experiment_type'] = 'Exp B: Freeze Deep Layers'
    config['hypothesis'] = 'Deep layer over-change causes Train/Val gap'
    with open(exp_dir / 'config.json', 'w') as f:
        json.dump(config, f, indent=2)

    print("=" * 60)
    print(f"Exp 1231_1: 凍結深層實驗 (Exp B)")
    print(f"Experiment: {args.exp_name}")
    print("=" * 60)
    print(f"LoRA Config:")
    print(f"  Layer preset: {args.lora_layers}")
    print(f"  Rank: {args.lora_rank}, Alpha: {args.lora_alpha}")
    print(f"  Target layers: {len(LORA_LAYER_PRESETS[args.lora_layers])} / 18")
    print("=" * 60)
    print(f"Hypothesis:")
    print(f"  深層 (L9-L17) 過度變化導致 overfitting")
    print(f"  凍結深層後，Val acc 應該提升")
    print("=" * 60)
    ...
    # Create model - 使用新的 ShallowLoRA 模型
    print("\n創建模型...")
    model = TeacherStudentShallowLoRA(
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        print(f"       Train-Val Gap={train_val_gap*100:.2f}%")  # 追蹤 gap
