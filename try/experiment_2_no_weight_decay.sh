...
# ========================================
# 假設: Weight Decay=0.01 抑制權重成長，減緩訓練
# 預期: Weight Decay=0 應能更快收斂

set -e

echo "🔬 實驗 2: 移除 Weight Decay (Weight Decay=0)"
echo "========================================"
...
# 實驗編號
EXP_ID="exp2_no_weight_decay_$(date +%Y%m%d_%H%M%S)"
...
NUM_EPOCHS=200
LEARNING_RATE=3e-4
WEIGHT_DECAY=0         # ⭐ 關鍵變數: 移除 Weight Decay
D_MODEL=512
NHEAD=8
NUM_LAYERS=4
DROPOUT=0.3            # 保持與原版一致
...
echo ""
echo "📊 實驗配置:"
echo "  Weight Decay: $WEIGHT_DECAY (vs 原版 0.01) ⭐"
echo "  Dropout: $DROPOUT (保持不變)"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "  - 與原版 (Weight Decay=0.01) 比較收斂速度"
    echo ""
    echo "📁 結果位置："
    echo "  - 日誌: $LOG_FILE"
    echo "  - 輸出: $OUTPUT_DIR"
    ...
    echo "❌ 實驗 2 失敗 (exit code: $EXIT_CODE)"
fi
echo "========================================"
