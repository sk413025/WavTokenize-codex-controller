...
# 引入離散編碼損失函數
from discrete_loss import compute_discrete_l2_loss, compute_discrete_content_consistency_loss, compute_hybrid_discrete_loss
import argparse
...
def compute_hybrid_loss_with_content(output, target_wav, enhanced_features, target_features, 
    ...
    # 分層內容一致性損失：取 residual block 第二層（索引1）
    if isinstance(intermediate_enhanced_features, list):
        if len(intermediate_enhanced_features) > 1:
            content_consistency_loss = compute_content_consistency_loss(intermediate_enhanced_features[1], content_ids, device)
        else:
...
def train_model(model, train_loader, optimizer, device, save_dir, config, num_epochs=100, scheduler=None, val_loader=None, use_content_loss=True):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (input_wav, target_wav, *extra_data) in progress_bar:
            ...
            elif config.get('use_discrete_loss', False) and input_discrete_code is not None and target_discrete_code is not None:
                # 使用離散編碼損失 - 結合連續特徵損失和離散編碼損失
                # 先計算連續特徵的損失
                cont_loss, cont_loss_details = compute_hybrid_loss(
                    output, target_wav, enhanced_features, target_features, device
                )
                
                # 再計算離散編碼的損失
                disc_loss, disc_loss_details = compute_hybrid_discrete_loss(
                    input_discrete_code, target_discrete_code, content_ids, device,
                    alpha=0.1, beta=0.9  # 權重可以根據需要調整
                )
                
                # 結合連續和離散損失 (可以調整權重)
                continuous_weight = 0.7  # 連續特徵損失權重
                discrete_weight = 0.3    # 離散編碼損失權重
                
                # 綜合損失
                loss = continuous_weight * cont_loss + discrete_weight * disc_loss
                
                # 合併損失詳情
                loss_details = {
                    'feature_loss': cont_loss_details['feature_loss'],
                    'voice_loss': cont_loss_details['voice_loss'],
                    'discrete_l2_loss': disc_loss_details['discrete_l2_loss'],
                    'discrete_content_consistency_loss': disc_loss_details['discrete_content_consistency_loss'],
                    'continuous_weight': continuous_weight,
                    'discrete_weight': discrete_weight
                }
                
                print(f"\r使用混合連續+離散損失 - 特徵損失: {loss_details['feature_loss']:.4f}, 離散L2: {loss_details['discrete_l2_loss']:.4f}, 離散內容: {loss_details['discrete_content_consistency_loss']:.4f}", end='')
            ...
            # 如果有離散編碼損失，則累積它們
            if "discrete_l2_loss" in loss_details:
                if "total_discrete_l2_loss" not in locals():
                    total_discrete_l2_loss = 0.0
                total_discrete_l2_loss += loss_details["discrete_l2_loss"]
                
            if "discrete_content_consistency_loss" in loss_details:
                if "total_discrete_content_loss" not in locals():
                    total_discrete_content_loss = 0.0
                total_discrete_content_loss += loss_details["discrete_content_consistency_loss"]            # 監控 GPU 記憶體
        ...
        # 計算離散編碼損失平均值（如果有）
        avg_discrete_l2_loss = 0.0
        avg_discrete_content_loss = 0.0
        
        if 'total_discrete_l2_loss' in locals():
            avg_discrete_l2_loss = total_discrete_l2_loss / len(train_loader)
            
        if 'total_discrete_content_loss' in locals():
            avg_discrete_content_loss = total_discrete_content_loss / len(train_loader)
        ...
        # 如果使用了離散編碼損失，則輸出相關信息
        if config.get('use_discrete_loss', False):
            print(f'Discrete L2 Loss: {avg_discrete_l2_loss:.4f}, '
                  f'Discrete Content Loss: {avg_discrete_content_loss:.4f}, '
...
def main():
    ...
    parser.add_argument("--use_discrete_loss", action="store_true", help="使用離散編碼的L2和內容一致性損失，結合連續特徵損失，旨在提高泛化能力")
...
def compute_layered_hybrid_loss(output, target_wav, enhanced_features, target_features, 
    ...
    """
    簡化版分層損失函數 - 針對不同層級調整內容一致性損失和L2特徵損失的權重。
    前幾層專注於內容一致性損失，後幾層專注於L2特徵損失。
    支持基於訓練進度動態調整權重：隨訓練進行，逐漸降低內容一致性損失的權重，提高特徵對齊的權重。
    ...
    for i, layer_features in enumerate(intermediate_features_list):
        ...
        if first_two_blocks_only:
            # 只對第一層有內容一致性損失，其餘層內容損失權重為0，L2損失權重為1
        ...
        # 降低內容一致性權重：使用動態衰減因子 - 隨訓練進行逐渐降低內容一致性損失的權重，提高特徵對齊的權重
        # 初始內容衰減因子（訓練初期）與結束衰減因子（訓練後期）
        initial_content_decay = 0.4  # 訓練初期，內容權重較低（衰減至原來的40%）
        final_content_decay = 0.0   # 訓練後期，內容權重降至0（完全不考慮內容損失）
        
        # 基於當前epoch計算動態衰減因子
        # 使用平滑過渡：前30%訓練保持初始值，然後逐漸線性降低，最後30%保持最終值
        train_progress = current_epoch / max(total_epochs, 1)
        
        if train_progress < 0.3:  # 訓練前30%
            content_decay_factor = initial_content_decay
        elif train_progress > 0.7:  # 訓練後30%
            content_decay_factor = final_content_decay
        else:  # 中間過渡階段，線性減少
            # 將0.3-0.7範圍映射到0-1
            t = (train_progress - 0.3) / 0.4
            # 線性插值計算當前衰減因子
            content_decay_factor = initial_content_decay - t * (initial_content_decay - final_content_decay)
        ...
        if first_two_blocks_only:
            # 前兩層: 100% 內容一致性損失，0% L2損失
            # 後續層: 0% 內容一致性損失，100% L2損失
            # 這裡直接使用原始權重設定，不受衰減因子影響
            content_weight = raw_content_weight  # 1.0 if i < 2 else 0.0
            l2_weight = raw_l2_weight           # 0.0 if i < 2 else 1.0
        else:
            # 標準模式: 應用動態衰減因子
    ...
    # 加入最終層的損失 (給予更高權重以提升音質)
    final_l2_weight = 0.9  # 進一步增加終層特徵對齊損失的權重，更專注於音質提升
    total_loss = total_loss + final_l2_weight * final_l2_loss
    ...
    # 添加更詳細的日誌信息
    # print(f"內容損失: {avg_content_loss:.4f}, L2損失: {avg_l2_loss:.4f}, 終層L2損失: {final_l2_loss:.4f}")
...
def compute_decay_factor(current_epoch, total_epochs):
    ...
    Returns:
        float: 計算出的內容衰減因子
    """
    initial_decay = 0.4  # 訓練初期，內容權重較低（衰減至原來的40%）
    final_decay = 0.0   # 訓練後期，內容權重降至0（完全不考慮內容損失）
    
    # 基於當前epoch計算動態衰減因子
    # 使用平滑過渡：前20%訓練保持初始值，然後逐漸線性降低，最後40%保持最終值
    train_progress = current_epoch / max(total_epochs, 1)
    
    if train_progress < 0.2:  # 訓練前20%
        return initial_decay
    elif train_progress > 0.6:  # 訓練後40%
        return final_decay
    else:  # 中間過渡階段，線性減少
        # 將0.2-0.6範圍映射到0-1
        t = (train_progress - 0.2) / 0.4
        # 線性插值計算當前衰減因子
        return initial_decay - t * (initial_decay - final_decay)
