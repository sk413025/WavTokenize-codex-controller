...
try:
    ...
    from .losses import EncoderDistillationLoss, EmbDistillationLoss
...
class Trainer:
    ...
    def __init__(self, config, exp_name):
        ...
        # 創建 loss function
        # 新增 'emb_distillation' 模式：使用 encoder 原始輸出計算 Loss
        self.use_emb_distillation = getattr(config, 'distance_loss_mode', 'gumbel') == 'emb_distillation'
        
        if self.use_emb_distillation:
            # 使用新的 EmbDistillationLoss (直接監督 encoder 原始輸出)
            ...
            # 原有的 EncoderDistillationLoss (支持 soft/gumbel/ste/ce/margin 模式)
            self.criterion = EncoderDistillationLoss(
                feature_loss_weight=config.feature_loss_weight,
                soft_dist_loss_weight=config.soft_dist_loss_weight,
                vq_loss_weight=config.vq_loss_weight,
                correct_vq_loss_weight=getattr(config, 'correct_vq_loss_weight', 0.0),
                temperature=config.temperature,
            ...
            print(f"Correct VQ loss weight: {getattr(config, 'correct_vq_loss_weight', 0.0)}")
