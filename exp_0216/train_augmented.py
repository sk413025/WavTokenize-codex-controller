"""
exp_0216: 資料增強 + 降低 LoRA Rank 訓練腳本

基於 exp_0206/plan_ori/train_single_vq_ema.py 修改，核心變更：
1. 使用 AugmentedCurriculumDataset（SNR Remix, Random Gain, Random Crop, Time Stretch）
2. 降低 LoRA rank: 256 → 64（減少 overfitting）
3. 增加 weight_decay: 0.01 → 0.02（加強正則化）

科學目標：
- 資料增強能否突破 val MSE 0.037 的天花板？
- 降低 LoRA rank 能否延後 overfitting epoch（目前 70 左右）？
- 兩者組合的整體效果？

對照基線：
- Plan Ori: best val MSE = 0.0374 (epoch 71), final = 0.0418
- V2 (4-layer RVQ): best val MSE = 0.0371 (epoch 78), final = 0.0398

執行：
    # Short-run (1000 steps) - 驗證增強是否正常
    python exp_0216/train_augmented.py --mode step --steps 1000

    # Long-run (300 epochs) - 完整實驗
    python exp_0216/train_augmented.py --mode epoch --epochs 300

    # 使用不同增強設定
    python exp_0216/train_augmented.py --mode epoch --epochs 300 \\
        --snr_remix_prob 0.7 --random_crop_prob 0.4 --lora_rank 64
"""
...
from exp_0216.data_augmented import create_augmented_curriculum_dataloaders


# ============================================================
# Utility (來自 train_single_vq_ema.py)
# ============================================================
...
def plot_training_curves(history, output_dir, epoch):
    ...
    fig.suptitle(f'exp_0216: Augmented + LoRA-64 (Epoch {epoch})', fontsize=14)
    ...
    if history.get('feature_mse'):
        ...
        # 標記 Plan Ori 和 V2 的基線
        ax.axhline(y=0.0374, color='blue', linestyle=':', alpha=0.5, label='Plan Ori best=0.0374')
        ax.axhline(y=0.0371, color='green', linestyle=':', alpha=0.5, label='V2 best=0.0371')
...
def plot_step_metrics(metrics_history: list, output_dir: Path):
    ...
    fig.suptitle('exp_0216: Augmented + LoRA-64 (Short-run)', fontsize=14)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='exp_0216: Data Augmentation + LoRA-64 Training'
    )
    ...
    parser.add_argument('--weight_decay', type=float, default=0.02,
                        help='Weight decay (Plan Ori 用 0.01，此處加強到 0.02)')
    ...
    # LoRA — 降低 rank 以減少 overfitting
    parser.add_argument('--lora_rank', type=int, default=64,
                        help='LoRA rank (Plan Ori 用 256，此處降到 64)')
    parser.add_argument('--lora_alpha', type=int, default=128,
                        help='LoRA alpha (保持 alpha/rank = 2)')
    ...
    if args.output_dir:
        ...
        exp_dir = Path(f'exp_0216/runs/augmented_{args.mode}_{timestamp}')
    ...
    config['experiment'] = 'exp_0216_augmented_lora64'

    with open(exp_dir / 'config.json', 'w') as f:
        json.dump(config, f, indent=2)

    print("=" * 70)
    print("exp_0216: Data Augmentation + LoRA-64")
    print("=" * 70)
    ...
    print(f"  SNR Remix:    p={args.snr_remix_prob}, range=[{args.snr_remix_min}, {args.snr_remix_max}] dB")
    print(f"  Random Gain:  p={args.random_gain_prob}, ±{args.random_gain_db} dB")
    print(f"  Random Crop:  p={args.random_crop_prob}, min_ratio={args.random_crop_min_ratio}")
    print(f"  Time Stretch: p={args.time_stretch_prob}, range=[{args.time_stretch_min}, {args.time_stretch_max}]")
    ...
    if args.mode == 'step':
        ...
        if metrics_history:
            ...
            summary = {
                ...
                'baseline_reference': {
                    'plan_ori_best_val_mse': 0.0374,
                    'plan_ori_final_val_mse': 0.0418,
                    'v2_best_val_mse': 0.0371,
                    'v2_final_val_mse': 0.0398,
                    'note': 'exp_0216 目標: 突破 val MSE 0.037 天花板',
                },
            }
        ...
        # Summary JSON
        summary = {
            'experiment': 'exp_0216_augmented_lora64',
            'mode': 'epoch',
            ...
            'changes_vs_plan_ori': {
                ...
                'data_augmentation': ['SNR Remix', 'Random Gain', 'Random Crop', 'Time Stretch'],
            },
        }
        with open(exp_dir / 'summary.json', 'w') as f:
            json.dump(summary, f, indent=2)
