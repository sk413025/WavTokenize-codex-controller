...
class Trainer:
    ...
    def save_audio_samples(self, epoch, num_samples=3):
        ...
        try:
            ...
            for i in range(min(num_samples, len(self.train_loader))):
                ...
                # 保存原始 noisy 音頻
                noisy_path = epoch_audio_dir / f'train_{i+1}_noisy.wav'
                torchaudio.save(str(noisy_path), noisy_audio.cpu(), sample_rate)
                
                # 保存 clean 音頻
                clean_path = epoch_audio_dir / f'train_{i+1}_clean.wav'
                torchaudio.save(str(clean_path), clean_audio.cpu(), sample_rate)
            
            # 保存驗證樣本
            val_iter = iter(self.val_loader)
            for i in range(min(num_samples, len(self.val_loader))):
                batch = next(val_iter)
                noisy_audio = batch['noisy_audio'][:1].to(self.device)
                clean_audio = batch['clean_audio'][:1].to(self.device)
                ...
                # 保存原始 noisy 音頻
                noisy_path = epoch_audio_dir / f'val_{i+1}_noisy.wav'
                torchaudio.save(str(noisy_path), noisy_audio.cpu(), sample_rate)
                
                # 保存 clean 音頻
                clean_path = epoch_audio_dir / f'val_{i+1}_clean.wav'
                torchaudio.save(str(clean_path), clean_audio.cpu(), sample_rate)
            
            print(f"  🔊 Saved {num_samples} train + {num_samples} val audio samples to {epoch_audio_dir}")
