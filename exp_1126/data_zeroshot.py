...
def zeroshot_collate_fn(batch, wavtokenizer, device):
    ...
    for noisy_t, clean_t in zip(noisy_tokens_list, clean_tokens_list):
        ...
        if curr_noisy.shape[0] < max_token_len:
            ...
            curr_noisy = torch.nn.functional.pad(curr_noisy, (0, pad_size), value=PAD_TOKEN)

        if curr_clean.shape[0] < max_token_len:
            pad_size = max_token_len - curr_clean.shape[0]
            curr_clean = torch.nn.functional.pad(curr_clean, (0, pad_size), value=PAD_TOKEN)
...
def cached_collate_fn(batch):
    ...
    Args:
        batch: List of dict, 每個 dict 包含:
          ...
          - content_id: str/int
          - noisy_path: str (optional, for visualization)
          - clean_path: str (optional, for visualization)
    ...
    Returns:
        dict with keys:
          ...
          - speaker_embeddings: (B, speaker_dim)
          - content_ids: (B,)
          - noisy_path: list of str (optional, for visualization)
          - clean_path: list of str (optional, for visualization)
    """
    # 提取數據
    ...
    # 保留 audio paths (用於可視化)
    noisy_paths = [item.get('noisy_path', None) for item in batch]
    clean_paths = [item.get('clean_path', None) for item in batch]
    ...
    # 只在有 paths 時添加 (兼容性)
    if any(p is not None for p in noisy_paths):
        result['noisy_path'] = noisy_paths
    if any(p is not None for p in clean_paths):
        result['clean_path'] = clean_paths

    return result


def clean_only_collate_fn(batch):
    """
    Collate function for Stage 1 (clean-only cache without noisy tokens)
