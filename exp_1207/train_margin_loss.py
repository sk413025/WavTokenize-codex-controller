...
def train_epoch(model, dataloader, optimizer, scheduler, device, epoch,
                distance_matrix, margin, ce_weight, ce_temperature=0.1,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        total_margin_loss += losses['margin_loss'].item()
        ...
        pbar.set_postfix({
            ...
            'margin': f"{losses['margin_loss'].item():.4f}",
    ...
    return {
        ...
        'margin_loss': total_margin_loss / n_batches,
...
@torch.no_grad()
def validate(model, dataloader, device, distance_matrix, margin, ce_weight,
...
def plot_training_curves(history, exp_dir, epoch):
    ...
    # 2. Feature Loss
    ax = axes[0, 1]
    ax.plot(epochs, history['train_margin_loss'], 'b-', label='Train', linewidth=2)
    ax.plot(epochs, history['val_margin_loss'], 'r-', label='Val', linewidth=2)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('MSE')
    ...
    # 8. Loss Breakdown
    ax = axes[1, 3]
    ax.plot(epochs, history['train_margin_loss'], 'b-', label='Feature', linewidth=2)
...
def main():
    ...
    # Experiment
    parser.add_argument('--exp_name', type=str, default='exp17_margin_loss',
    ...
    print(f"  - Total = Margin Loss + {args.ce_weight}*CE")
    print("="*60 + "\n")
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        train_results = train_epoch(
            ...
            args.margin, args.ce_weight, args.ce_temperature,
        ...
        history['train_margin_loss'].append(train_results['margin_loss'])
        ...
        history['val_margin_loss'].append(val_results['margin_loss'])
