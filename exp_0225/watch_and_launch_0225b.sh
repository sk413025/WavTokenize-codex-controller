...
#
# 用法：bash exp_0225/watch_and_launch_0225b.sh
# 或背景執行：nohup bash exp_0225/watch_and_launch_0225b.sh > /tmp/watch_0225b.log 2>&1 &
...
while true; do
    ...
    LATEST_RUN=$(ls -dt "$BASE/exp_0225/runs/no_vq_scratch_epoch_"* 2>/dev/null | head -1 || true)
...
# -------------------------------------------------------
nohup "$CONDA_PYTHON" \
    "$BASE/exp_0225/train_no_vq_scratch_decoder_lora.py" \
    --mode epoch \
    --epochs 300 \
    --device "$DEVICE" \
