"""
exp_1223: Speaker-Conditioned Training Script

基於 exp_1219/train.py，新增:
1. Speaker embedding 輸入
2. Speaker conditioning (FiLM or Cross-Attention)
3. 支援從 Exp55/Exp59 checkpoint 微調
"""
...
from exp_1223.models_speaker import TeacherStudentSpeakerConditioned
from exp_1219.losses import (
    MaskedCombinedLossV2, compute_masked_accuracy, create_length_mask
)
...
def train_epoch(
    ...
    metrics = {
        ...
        'cosine_loss': 0, 'cos_sim_mean': 0, 'cos_sim_std': 0,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        speaker_embedding = batch['speaker_embedding'].to(device)  # NEW
        ...
        if use_amp and scaler is not None:
            with autocast(enabled=use_amp):
                # 傳入 speaker_embedding
                output = model(noisy_audio, clean_audio, speaker_embedding)

                logits = None
                if loss_fn.ce_weight > 0:
                    logits = model.base_model.compute_ce_logits(output['student_encoder_out'])
        ...
        metrics['cos_sim_std'] += loss_info.get('cos_sim_std', 0)
        ...
        cos_sim_display = loss_info.get('cos_sim_mean', 0)
        pbar.set_postfix({
            'loss': f"{loss.item():.4f}",
            'm_acc': f"{masked_acc*100:.1f}%",
            'cos': f"{cos_sim_display:.2f}"
        })

    for key in ['total_loss', 'feature_loss', 'triplet_loss', 'ce_loss',
                'cosine_loss', 'cos_sim_mean', 'cos_sim_std',
...
def save_audio_samples(model, dataloader, device, exp_dir, epoch, num_samples=3, split='val'):
    ...
    for i in range(min(num_samples, len(dataloader))):
        ...
        speaker_embedding = batch['speaker_embedding'][:1].to(device)
        ...
        try:
            # Get student features (with speaker conditioning)
            output = model(noisy_audio, clean_audio, speaker_embedding)
            student_features = output['student_encoder_out']

            # Decode student features to audio
            ...
            # Get teacher features and decode (for reference)
...
def plot_metrics(history, exp_dir):
    ...
    # Cosine Loss
    ax = axes[0, 3]
    if history.get('train_cosine_loss', []):
        ax.plot(history['train_cosine_loss'], label='Train')
        ax.plot(history['val_cosine_loss'], label='Val')
    ax.set_xlabel('Epoch')
...
def main():
    ...
    parser.add_argument('--speaker_condition_type', type=str, default='film',
                        choices=['film', 'cross_attention'],
                        help='Speaker conditioning method')
    ...
    parser.add_argument('--pretrained', type=str, default=None,
                        help='Path to pretrained checkpoint (e.g., Exp55/Exp59)')
    ...
    print("=" * 60)
    print(f"exp_1223: Speaker-Conditioned Training")
    print(f"Experiment: {args.exp_name}")
    print("=" * 60)
    print(f"LoRA Config:")
    print(f"  Rank: {args.lora_rank}, Alpha: {args.lora_alpha}")
    print(f"  Dropout: {args.lora_dropout}, Layers: {args.lora_layers}")
    print(f"Speaker Conditioning:")
    print(f"  Type: {args.speaker_condition_type}")
    print(f"Loss Config:")
    ...
    train_loader, val_loader = create_speaker_aware_dataloaders(DataConfig())
    ...
    # 創建模型
    print("\n創建模型...")
    model = TeacherStudentSpeakerConditioned(
        ...
        speaker_condition_type=args.speaker_condition_type,
        device=device,
    )

    # Load pretrained weights (LoRA only, without speaker condition)
    if args.pretrained:
        pretrained_path = Path(args.pretrained)
        if pretrained_path.exists():
            print(f"\n載入 pretrained checkpoint: {pretrained_path}")
            checkpoint = torch.load(pretrained_path, map_location=device)

            # 只載入 base_model (LoRA) 的權重
            state_dict = checkpoint.get('model_state_dict', checkpoint)
            base_model_state = {
                k.replace('base_model.', ''): v
                for k, v in state_dict.items()
                if not k.startswith('speaker_condition')
            }
            model.base_model.load_state_dict(base_model_state, strict=False)
            print(f"  LoRA weights loaded from pretrained")
    ...
    history = {
        ...
        'train_cosine_loss': [], 'val_cosine_loss': [],
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        history['train_cosine_loss'].append(train_metrics['cosine_loss'])
        history['val_cosine_loss'].append(val_metrics['cosine_loss'])
        history['train_cos_sim'].append(train_metrics['cos_sim_mean'])
        history['val_cos_sim'].append(val_metrics['cos_sim_mean'])
