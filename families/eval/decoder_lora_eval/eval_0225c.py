"""
families/eval/decoder_lora_eval/eval_0225c.py

評估 exp_0225c 並更新 comparison_results.json 和 comparison_table.md：
  - exp_0225c: best_model.pt (epoch=5, val_wav_mse=0.02768)
    No-VQ Decoder LoRA + Phase Loss，encoder init from exp_0225a

執行：
    cd /home/sbplab/ruizi/WavTokenize-feature-analysis
    PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \
    /home/sbplab/miniconda3/envs/test/bin/python families/eval/decoder_lora_eval/eval_0225c.py
"""
...
EXP_0225C = {
    'name': 'exp_0225c',
    'folder': 'exp_0225c',
    'ckpt': 'families/deps/no_vq_scratch/runs/no_vq_scratch_dec_lora_phase_epoch_20260225_012841/best_model.pt',
    'lora_rank': 64, 'lora_alpha': 128,
    'decoder_lora_rank': 32, 'decoder_lora_alpha': 64,
    'note': 'best_model.pt (val_wav_mse criterion, epoch=5, val_wav_mse=0.02768; encoder from 0225a; +phase loss)',
}
...
def rebuild_comparison_table(all_results):
    ...
    lines.append("- **exp_0225b**：No-VQ+DecLoRA，encoder from 0225a，ep33")
    lines.append("- **exp_0225c**：No-VQ+DecLoRA+Phase Loss，encoder from 0225a，ep5（phase loss 無助改善，負結果）")
...
def main():
    ...
    try:
        metrics_list = run_exp(exp, loader, out_dir)
