"""
families/eval/decoder_lora_eval/eval_0228fm_0229b.py

評估 exp_0228_fm 和 exp_0229b，更新 comparison_results.json 和 comparison_table.md：

  - exp_0228_fm: best_model_val_total.pt (epoch=77; EncOnly+FeatAlign+MRD-FM+HuBERT)
  - exp_0229b:   checkpoint_epoch140.pt  (epoch=140; LatentBWE on frozen enc_0227)

執行：
    cd /home/sbplab/ruizi/WavTokenize-feature-analysis
    PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \
    /home/sbplab/miniconda3/envs/test/bin/python families/eval/decoder_lora_eval/eval_0228fm_0229b.py
"""
...
def run_exp_0228fm(loader, out_dir):
    """執行 exp_0228_fm 評估（TeacherStudentNoVQ + HuBERT loss）。
    ...
    ckpt_path = BASE / 'families/official/hubert_then_distalign/runs/enc_hubert_fm_epoch_20260301_082830/best_model_val_total.pt'
    print(f"  Loading exp_0228_fm from: {ckpt_path}")
...
def run_exp_0229b(loader, out_dir):
    """執行 exp_0229b 評估（LatentBWEPipeline，frozen enc_0227 + trainable LatentBWE）。
    ...
    Returns:
        per-sample metrics list。
    """
    # import LatentBWEPipeline from training script
    sys.path.insert(0, str(BASE / 'exp_0229b'))
    from train_bwe_latent import LatentBWEPipeline

    enc_ckpt = BASE / 'families/deps/enc_mrd_fm/runs/enc_mrd_fm_epoch_20260227_024953/best_model_val_total.pt'
    bwe_ckpt = BASE / 'families/eval/bwe_latent/runs/bwe_latent_epoch_20260301_094617/checkpoint_epoch140.pt'
    print(f"  Loading exp_0229b")
    print(f"    enc_ckpt: {enc_ckpt}")
    print(f"    bwe_ckpt: {bwe_ckpt} (best epoch=140)")
...
def main():
    """主評估流程：執行 exp_0228_fm 和 exp_0229b，更新 JSON 和 MD。"""
    ...
    experiments = [
        ('exp_0228_fm', run_exp_0228fm, 'exp_0228_fm',
         'best_model_val_total.pt (epoch=77; EncOnly+FeatAlign+MRD-FM+HuBERT)'),
        ('exp_0229b', run_exp_0229b, 'exp_0229b',
         'checkpoint_epoch140.pt (epoch=140; LatentBWE on frozen enc_0227)'),
    ]
