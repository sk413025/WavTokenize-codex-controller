"""
families/eval/decoder_lora_eval/eval_new_exps.py

評估新實驗並更新 comparison_results.json 和 comparison_table.md：
  - exp_0224b_best: best_model.pt (epoch=142, val_mse=0.02744)  替換 ep40/ep50
  - exp_0225a:      best_model_val_total.pt (val_total=35.50)

執行：
    cd /home/sbplab/ruizi/WavTokenize-feature-analysis
    PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \
    /home/sbplab/miniconda3/envs/test/bin/python families/eval/decoder_lora_eval/eval_new_exps.py
"""
...
NEW_EXPERIMENTS = [
    {
        ...
        'note': 'best_model.pt (val_mse criterion, epoch=142, val_mse=0.02744)',
    },
    {
        'name': 'exp_0225a',
        'folder': 'exp_0225a',
        'type': 'no_vq_scratch',
        'ckpt': 'families/deps/no_vq_scratch/runs/no_vq_scratch_epoch_20260224_032104/best_model_val_total.pt',
        'lora_rank': 64, 'lora_alpha': 128,
        'note': 'best_model_val_total.pt (val_total=35.50, scratch encoder, no exp_0217 prior)',
    },
]
...
def run_no_vq_scratch(exp, loader, out_dir):
    """exp_0225a: No-VQ encoder trained from scratch (no exp_0217 prior)"""
    from families.deps.no_vq_core.models_no_vq import TeacherStudentNoVQ

    print(f"  Loading {exp['name']} (No-VQ scratch encoder)...")
...
def rebuild_comparison_table(all_results):
    """重建 comparison_table.md（保留已有說明格式）"""
    ...
    LABELS = {
        ...
        'exp_0224b_best': '**exp_0224b_best** (No-VQ+DecLoRA, ep142)',
    ...
    lines.append("- **exp_0224b_best**（最終最佳 ep142）：替換先前的 ep40/ep50 中間結果")
    lines.append("- **exp_0225a**：從頭訓練（無 exp_0217 VQ token 預訓練），消融實驗基準")
...
def main():
    ...
    # 移除 ep40 / ep50
    for key in ['exp_0224b_ep40', 'exp_0224b_ep50']:
    ...
    for exp in NEW_EXPERIMENTS:
        ...
        try:
            ...
            elif exp['type'] == 'no_vq_scratch':
                metrics_list = run_no_vq_scratch(exp, loader, out_dir)
            else:
