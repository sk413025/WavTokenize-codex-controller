"""
exp_0224: No-VQ Encoder LoRA Fine-tune

架構：
    Encoder LoRA (Trainable, 繼承 exp_0217):
...
對比實驗：
    exp_0223: Decoder LoRA, Encoder+VQ frozen → 改善 decoder 適應 student tokens
    exp_0224: Encoder LoRA, VQ 跳過, Decoder frozen → 直接學 continuous feature → clean wav

科學問題：
    VQ 量化瓶頸是否是主要障礙？
    跳過 VQ 讓 encoder 直接對齊 decoder input space 是否能突破 PESQ ceiling？

執行：
    # Smoke test (5 epochs, 20 samples)
    python families/deps/no_vq_core/train_no_vq.py --mode smoke

    # 正式訓練
    python families/deps/no_vq_core/train_no_vq.py --mode epoch --epochs 150 --device cuda:1
"""
...
def train_epoch(model, dataloader, optimizer, device, epoch, config,
    ...
    """
    model.train()
    # teacher decoder frozen，但 backbone 需要 eval mode（無 dropout）
    model.teacher.backbone.eval()
    model.teacher.head.eval()
    # student encoder LoRA 在 train mode（有 LoRA dropout）
    model.student.train()
    ...
    pbar = tqdm(dataloader, desc=f"Epoch {epoch} [no-VQ encoder LoRA]")
...
def plot_training_curves(history, output_dir, epoch):
    ...
    fig.suptitle(f'exp_0224: No-VQ Encoder LoRA (Epoch {epoch})', fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description='exp_0224: No-VQ Encoder LoRA + MR-STFT + Mel Loss'
    )
    ...
    # Encoder checkpoint（繼承 exp_0217）
    parser.add_argument(
    ...
    if args.output_dir:
        ...
        exp_dir = Path(f'families/deps/no_vq_core/runs/no_vq_{args.mode}_{timestamp}')
    ...
    config['experiment'] = 'exp_0224_no_vq_encoder_lora'

    with open(exp_dir / 'config.json', 'w') as f:
        json.dump(config, f, indent=2)

    print("=" * 70)
    print("exp_0224: No-VQ Encoder LoRA + MR-STFT + Mel Loss")
    print("=" * 70)
    ...
    print(f"Loss: λ_wav={args.lambda_wav} * wav_mse + λ_stft={args.lambda_stft} * MR-STFT + λ_mel={args.lambda_mel} * mel")
    ...
    # ===== Load Encoder Checkpoint (exp_0217) =====
    ...
    if encoder_ckpt_path.exists():
        ...
        print("  Starting with fresh encoder LoRA")
    ...
    print(f"  Sample trainable: {trainable_names[:3]}")
