"""
exp_0225: No-VQ Encoder LoRA Fine-tune（從 WavTokenizer 預訓練起點）

對照實驗：exp_0224a 從 exp_0217 best_model.pt 繼承 encoder weights；
          exp_0225  從 WavTokenizer 預訓練 encoder 直接開始，不載入任何 LDV 先驗。

架構（與 exp_0224a 完全相同）：
    Encoder LoRA (Trainable, WavTokenizer 預訓練起點):
...
科學問題：
    exp_0217 的 LDV 先驗知識（Phase 1）是否有助於 exp_0224a 的收斂？
    若 exp_0225 PESQ < exp_0224a，說明 Phase 1 有貢獻（漸進式訓練有效）。
    若 exp_0225 PESQ ≈ exp_0224a，說明 Phase 1 可省略。

執行：
    # Smoke test (5 epochs, 20 samples)
    PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \\
    /home/sbplab/miniconda3/envs/test/bin/python families/deps/no_vq_scratch/train_no_vq_scratch.py --mode smoke

    # 正式訓練
    PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \\
    /home/sbplab/miniconda3/envs/test/bin/python families/deps/no_vq_scratch/train_no_vq_scratch.py \\
...
sys.path.insert(0, str(Path(__file__).parent.parent / 'exp_0224'))
...
def train_epoch(model, dataloader, optimizer, device, epoch, config,
    ...
    pbar = tqdm(dataloader, desc=f"Epoch {epoch} [no-VQ scratch]")
...
def plot_training_curves(history, output_dir, epoch):
    ...
    fig.suptitle(f'exp_0225: No-VQ Encoder LoRA from Scratch (Epoch {epoch})', fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description='exp_0225: No-VQ Encoder LoRA from WavTokenizer scratch'
    )
    ...
    if args.output_dir:
        ...
        exp_dir = Path(f'families/deps/no_vq_scratch/runs/no_vq_scratch_{args.mode}_{timestamp}')
    ...
    config['experiment'] = 'exp_0225_no_vq_scratch'
    config['encoder_init'] = 'WavTokenizer pretrained (no prior LDV checkpoint)'

    with open(exp_dir / 'config.json', 'w') as f:
        json.dump(config, f, indent=2)

    print("=" * 70)
    print("exp_0225: No-VQ Encoder LoRA + MR-STFT + Mel Loss (from scratch)")
    print("=" * 70)
    ...
    print(f"Decoder: FROZEN (original pretrained)")
    print(f"Encoder init: WavTokenizer pretrained + fresh LoRA (NO exp_0217 prior)")
    print(f"Loss: λ_wav={args.lambda_wav} + λ_stft={args.lambda_stft} + λ_mel={args.lambda_mel}")
    ...
    print("\nBuilding TeacherStudentNoVQ (from WavTokenizer pretrained, no prior)...")
    ...
    print("  Encoder initialized from WavTokenizer pretrained weights + fresh LoRA")
    print("  (No exp_0217 checkpoint loaded — this is the key difference from exp_0224a)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_total < best_val_total:
            ...
            torch.save(best_total_ckpt, exp_dir / 'best_model_val_total.pt')
