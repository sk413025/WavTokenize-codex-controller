"""
exp_0225b: No-VQ + Decoder LoRA Fine-tune（消融實驗：從頭開始的 Encoder）
...
消融實驗設計（對應 exp_0224b）：
    exp_0224a:  Encoder = exp_0217 → Phase2 訓練（繼承 Phase1 prior）
    exp_0224b:  Encoder = exp_0224a（凍結）→ Decoder LoRA  ← 三階段系統
    exp_0225a:  Encoder = WavTokenizer scratch（無 Phase1 prior）→ Phase2 訓練
    exp_0225b:  Encoder = exp_0225a（凍結）→ Decoder LoRA  ← 本腳本（消融對照）

科學問題：
    Phase 1（exp_0217 的 VQ token 預訓練）是否對最終 PESQ 有顯著貢獻？
    比較 exp_0224b 和 exp_0225b 的結果即可量化 Phase 1 的貢獻。

執行順序（必須先跑完 exp_0225a）：
    # Step 1: 等 exp_0225a 完成（train_no_vq_scratch.py, 300 epochs）

    # Step 2: 用 exp_0225a best_model_val_total.pt 啟動 exp_0225b
    python families/deps/no_vq_scratch/train_no_vq_scratch_decoder_lora.py \
    ...
    # Smoke test
    python families/deps/no_vq_scratch/train_no_vq_scratch_decoder_lora.py --mode smoke
"""
...
# exp_0224a 訓練完成後路徑會在 families/deps/no_vq_core/runs/no_vq_epoch_*/best_model.pt
# 啟動時請用 --encoder_ckpt 指定實際路徑
# exp_0225a best_model_val_total.pt（exp_0225b 的 encoder 起點）
# 監控腳本會自動傳入 --encoder_ckpt；手動執行請指定實際路徑
EXP0225A_BEST_CKPT_GLOB = 'families/deps/no_vq_scratch/runs/no_vq_scratch_epoch_*/best_model_val_total.pt'

# fallback: 若 exp_0225a 尚未完成（不應發生，由監控腳本保證）
EXP0217_BEST_CKPT = (
...
def train_epoch(model, dataloader, optimizer, device, epoch, config,
    ...
    pbar = tqdm(dataloader, desc=f"Epoch {epoch} [no-VQ decoder LoRA]")
...
def plot_training_curves(history, output_dir, epoch):
    ...
    fig.suptitle(f'exp_0224b: No-VQ + Decoder LoRA (Epoch {epoch})', fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description='exp_0224b: No-VQ + Decoder LoRA'
    )
    ...
    parser.add_argument('--encoder_ckpt', type=str, default=str(EXP0217_BEST_CKPT),
                        help='Encoder checkpoint 路徑。應指定 exp_0225a best_model_val_total.pt，'
                             '例如: families/deps/no_vq_scratch/runs/no_vq_scratch_epoch_*/best_model_val_total.pt')
    ...
    if args.output_dir:
        ...
        exp_dir = Path(f'families/deps/no_vq_scratch/runs/no_vq_scratch_dec_lora_{args.mode}_{timestamp}')
    ...
    config['experiment'] = 'exp_0225b_no_vq_scratch_decoder_lora'

    with open(exp_dir / 'config.json', 'w') as f:
        json.dump(config, f, indent=2)

    print("=" * 70)
    print("exp_0225b: No-VQ + Decoder LoRA (ablation: encoder from scratch)")
    print("=" * 70)
    ...
    print(f"Encoder LoRA: rank={args.lora_rank} (FROZEN, from exp_0225a = WavTokenizer scratch)")
    print(f"VQ: SKIPPED")
