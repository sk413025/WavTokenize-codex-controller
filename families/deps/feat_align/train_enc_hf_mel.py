"""
exp_0226b: No-VQ Encoder LoRA + Feature Alignment + 高頻強調 Mel Loss

與 exp_0226a 的差異：
    在 MelReconstructionLoss 內，對高頻 mel bin（> hf_mel_bin_start，預設 bin 40/100）
    額外乘以 hf_mel_weight（預設 3.0）放大高頻 mel 的梯度貢獻。

動機：
    LDV 感測器物理上缺失 2kHz 以上的高頻。
    exp_0226a 的 Mel 圖已觀察到 encoder 嘗試長出高頻，但力道不足。
    高頻強調 loss 可在不改變架構的情況下，引導 encoder 更努力重建高頻諧波。
...
Loss：
    ...
    + λ_mel  × Mel(recon_wav, clean_wav)          ← 含高頻強調
    + λ_feat × MSE(student_feat, teacher_feat)
    + λ_hf   × HF_Mel(recon_wav, clean_wav)       ← 只取 bin[hf_start:]，額外監督

執行：
    PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \\
    /home/sbplab/miniconda3/envs/test/bin/python families/deps/feat_align/train_enc_hf_mel.py --mode smoke

    PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \\
    /home/sbplab/miniconda3/envs/test/bin/python families/deps/feat_align/train_enc_hf_mel.py \\
...
class HighFreqMelLoss(nn.Module):
    """
    高頻強調 Mel Loss：只計算 mel bin[hf_bin_start:] 的 L1 loss。

    n_mels=100, sample_rate=24000 時，mel bin 與頻率的大致對應：
        bin 0  ≈    0 Hz
        bin 20 ≈  630 Hz
        bin 40 ≈ 1600 Hz   ← 預設 hf_bin_start=40（~1.6kHz 以上）
        bin 60 ≈ 3600 Hz
        bin 80 ≈ 7000 Hz
        bin 99 ≈ 12000 Hz

    LDV 高頻物理缺失 > 2kHz；bin 40（~1.6kHz）稍低於此，
    涵蓋頻帶延伸的過渡區域，讓 loss 有足夠的梯度信號。
    """
    def __init__(self, sample_rate=24000, n_fft=1024, hop_length=256,
                 n_mels=100, hf_bin_start=40):
        super().__init__()
        self.mel_spec = torchaudio.transforms.MelSpectrogram(
            sample_rate=sample_rate, n_fft=n_fft, hop_length=hop_length,
            n_mels=n_mels, center=True, power=1,
    ...
    def forward(self, y_hat: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        mel = torch.log(self.mel_spec(y).clamp(min=1e-7))
        # 只取高頻 bin
        return F.l1_loss(mel_hat[:, self.hf_bin_start:, :], mel[:, self.hf_bin_start:, :])


# ============================================================
# Utilities（與 exp_0226a 相同）
# ============================================================
...
def train_epoch(model, dataloader, optimizer, device, epoch, config,
                mr_stft_loss_fn, mel_loss_fn, hf_mel_loss_fn, scaler=None) -> Dict:
    model.train()
    ...
    pbar = tqdm(dataloader, desc=f"Epoch {epoch} [enc+hf-mel]")
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with autocast(enabled=config['use_amp']):
            ...
            teacher_feat = out['teacher_encoder_out']   # [B, 512, T]
            ...
            # 高頻 Mel loss（bin[hf_bin_start:]）
            hf_mel_loss = hf_mel_loss_fn(recon_t, clean_t)
        ...
        metrics['hf_mel_loss']    += hf_mel_loss.item()
        n_batches += 1
        ...
        pbar.set_postfix({
            ...
            'hf_mel': f"{hf_mel_loss.item():.4f}",
        })

    if n_batches > 0:
        for k in ['total_loss', 'wav_mse', 'stft_sc_loss', 'stft_mag_loss',
                  'mel_loss', 'feat_align_loss', 'hf_mel_loss']:
            metrics[k] /= n_batches
...
def evaluate(model, dataloader, device, config,
    ...
    for i, batch in enumerate(dataloader):
        ...
        hf_mel_list.append(hf_mel_loss_fn(recon_t, clean_t).item())
    ...
    return {
        ...
        'val_hf_mel_loss':   float(np.mean(hf_mel_list))        if hf_mel_list else float('nan'),
...
def plot_training_curves(history, output_dir, epoch):
    ...
    fig.suptitle(f'exp_0226b: Encoder LoRA + FeatAlign + HF Mel (Epoch {epoch})', fontsize=14)
    ...
    ax = axes[1, 0]
    if history.get('train_hf_mel'):
        ax.plot(epochs, history['train_hf_mel'], 'orange', label='Train HF Mel', alpha=0.8)
    if history.get('val_hf_mel_loss'):
        ax.plot(epochs, history['val_hf_mel_loss'], 'red', ls='--', label='Val HF Mel', alpha=0.8)
    if history.get('train_mel'):
        ax.plot(epochs, history['train_mel'], 'b-', label='Train Mel (full)', alpha=0.5)
    if history.get('val_mel_loss'):
        ax.plot(epochs, history['val_mel_loss'], 'b--', label='Val Mel (full)', alpha=0.5)
    ax.set_title('Mel Loss: Full vs High-Freq (train & val)')
    ax.set_xlabel('Epoch')
...
def main():
    parser = argparse.ArgumentParser(
        description='exp_0226b: Encoder LoRA + Feature Alignment + HF Mel Loss'
    )
    ...
    parser.add_argument('--encoder_ckpt', type=str, default=EXP0225A_BEST_CKPT,
                        help='Base encoder ckpt (預設 exp_0225a best_model_val_total.pt)')
    ...
    parser.add_argument('--lambda_hf', type=float, default=45.0,
                        help='高頻 Mel loss weight（預設與全頻 Mel 相同）')

    parser.add_argument('--hf_bin_start', type=int, default=40,
                        help='高頻 Mel bin 起始（n_mels=100 時，40 ≈ 1.6kHz）')
    ...
    if args.output_dir:
        ...
        exp_dir = Path(f'families/deps/feat_align/runs/enc_hf_mel_{args.mode}_{timestamp}')
    ...
    config['experiment'] = 'exp_0226b_enc_hf_mel'
    ...
    print("=" * 70)
    print("exp_0226b: Encoder LoRA + Feature Alignment + HF Mel Loss")
    print("=" * 70)
    ...
    print(f"Loss: λ_wav={args.lambda_wav} + λ_stft={args.lambda_stft} "
          ...
          f"+ λ_hf={args.lambda_hf} (bin>={args.hf_bin_start})")
    ...
    # 載入 base encoder LoRA 權重（預設 exp_0225a）
    ...
    hf_mel_loss_fn = HighFreqMelLoss(
        sample_rate=SAMPLE_RATE, n_fft=1024, hop_length=256,
        n_mels=100, hf_bin_start=args.hf_bin_start,
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_metrics = train_epoch(
            ...
            mr_stft_loss_fn, mel_loss_fn, hf_mel_loss_fn, scaler,
        )
        val_metrics = evaluate(
            model, val_loader, device, config,
            mr_stft_loss_fn, mel_loss_fn, hf_mel_loss_fn, args.eval_max_batches,
        )
        ...
        history['train_hf_mel'].append(train_metrics['hf_mel_loss'])
        ...
        history['val_hf_mel_loss'].append(val_metrics['val_hf_mel_loss'])
        ...
        print(f"  Train: total={train_metrics['total_loss']:.4f}  "
              ...
              f"hf_mel={train_metrics['hf_mel_loss']:.4f}")
        ...
        print(f"         feat_align={val_metrics['val_feat_align']:.5f}  "
              f"hf_mel={val_metrics['val_hf_mel_loss']:.4f}  "
