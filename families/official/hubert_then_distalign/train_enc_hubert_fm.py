"""
exp_0228: No-VQ Encoder LoRA + Feature Alignment + Frozen MRD FM + Frozen HuBERT Feature Loss

與 exp_0227 的差異：
    新增 λ_hubert × HuBERT_Feature_Loss
    使用 facebook/hubert-base-ls960（完全凍結）作為語音語意監督，
    提取 recon_wav 和 clean_wav 在 HuBERT 中間層的特徵距離。

動機：
    exp_0227 使用 MRD FM 提供「音質感知」梯度（自然度、頻譜結構），
    但缺乏「語音內容」監督 → 咬字不清、子音容易被 MSE/Mel loss 忽略。
    HuBERT 中間層（layer 6-8）編碼了豐富的語音學特徵（phonetic features），
    能有效監督發音清晰度，特別是低能量的子音（consonants, 2-8kHz）。

    MRD FM 與 HuBERT Loss 互補：
    - MRD FM → 自然度、頻譜真實感（perceptual quality）
    - HuBERT → 語音內容保真度（phonetic fidelity / articulation）

HuBERT 結構：
    facebook/hubert-base-ls960，94.4M params，12 層 Transformer
    input: 16kHz mono waveform → output: 50Hz frame-level features (hidden_size=768)
    使用 output_hidden_states=True 取得所有 13 層 hidden states
    取 layer 6, 7, 8 的均值做語音學特徵（中間層 = 語音學資訊最豐富）
...
架構：
    ...
    HuBERT (Frozen, facebook/hubert-base-ls960, 僅用於 phonetic supervision)
...
Loss：
    ...
    + λ_fm   × L1_mean( MRD_fmap(recon), MRD_fmap(clean) )
    + λ_hubert × L1( HuBERT_mid(recon_16k), HuBERT_mid(clean_16k) )  ← 新增

執行：
    PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \\
    /home/sbplab/miniconda3/envs/test/bin/python families/official/hubert_then_distalign/train_enc_hubert_fm.py --mode smoke

    PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \\
    /home/sbplab/miniconda3/envs/test/bin/python families/official/hubert_then_distalign/train_enc_hubert_fm.py \\
...
def train_epoch(model, mrd_fm_loss_fn, hubert_loss_fn, dataloader, optimizer,
    ...
    pbar = tqdm(dataloader, desc=f"Epoch {epoch} [enc+mrd-fm+hubert]")
    ...
    if n_batches > 0:
        for k in ['total_loss', 'wav_mse', 'stft_sc_loss', 'stft_mag_loss',
                  'mel_loss', 'feat_align_loss', 'mrd_fm_loss', 'hubert_loss']:
            metrics[k] /= n_batches
...
def evaluate(model, mrd_fm_loss_fn, hubert_loss_fn, dataloader, device, config,
    ...
    feat_align_list, mrd_fm_list, hubert_list = [], [], []
...
def plot_training_curves(history, output_dir, epoch):
    ...
    fig.suptitle(f'exp_0228: EncOnly + FeatAlign + MRD FM + HuBERT (Epoch {epoch})', fontsize=14)
    ...
    ax.axis('off')
    summary_text = (
        f"exp_0228: Encoder LoRA + FeatAlign + MRD FM + HuBERT\n"
        f"Epoch: {epoch}\n"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='exp_0228: Encoder LoRA + FeatAlign + MRD FM + HuBERT Feature Loss'
    )
    ...
    if args.output_dir:
        ...
        exp_dir = Path(f'families/official/hubert_then_distalign/runs/enc_hubert_fm_{args.mode}_{timestamp}')
    ...
    config['experiment'] = 'exp_0228_enc_hubert_fm'
    ...
    print("=" * 70)
    print("exp_0228: Encoder LoRA + FeatAlign + MRD FM + HuBERT Feature Loss")
    print("=" * 70)
