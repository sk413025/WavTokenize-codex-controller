"""
exp_0128: TracIn-Weighted Soft Reweighting Short-Run (實驗 1)

目的：
- 驗證「TracIn 軟性重加權」是否能改善 token collapse
- Short-run: 800-1000 steps
- 基於 exp_k v6 baseline 配置

策略：
- 使用 TracIn scores 計算權重：weight = 1 / (1 + α × score)
- High-influence proponents → 低權重 (down-weighted)
- Low-influence opponents → 高權重 (up-weighted)
...
執行：
    CUDA_VISIBLE_DEVICES=1 python exp_0128/soft_reweighting/train_short_run.py \
        --steps 1000 \
        --batch_size 2 \
        --alpha 0.5 \
        --output_dir exp_0128/soft_reweighting/run_short_alpha0.5 \
        --seed 42
"""
...
from exp_0128.soft_reweighting.data_weighted import create_tracin_weighted_dataloaders
...
def plot_loss_curves(loss_history, save_path):
    ...
    fig.suptitle('Exp 0128-1: TracIn-Weighted Soft Reweighting', fontsize=12)
...
def main():
    parser = argparse.ArgumentParser(description='Exp 0128 - TracIn-Weighted Soft Reweighting Short-Run')
    ...
    parser.add_argument('--alpha', type=float, default=0.5,
                        help='TracIn reweighting strength (0.3/0.5/0.7)')
    parser.add_argument('--tracin_scores_csv', type=str,
                        default='exp_0125/tracin_token_collapse_589e6d/tracin_scores_5ckpt.csv',
                        help='Path to TracIn scores CSV')
    parser.add_argument('--output_dir', type=str, default='exp_0128/soft_reweighting/run_short',
    ...
    # Save config
    config = vars(args)
    config['experiment'] = 'exp_0128_tracin_weighted_soft_reweighting'
    ...
    print("=" * 60)
    print("Exp 0128: TracIn-Weighted Soft Reweighting Short-Run")
    print("=" * 60)
    print(f"Steps: {args.steps}")
    print(f"Batch size: {args.batch_size}")
    print(f"Grad accum: {args.grad_accum}")
    print(f"Effective batch size: {args.batch_size * args.grad_accum}")
    print(f"Alpha (reweighting strength): {args.alpha}")
    print(f"TracIn scores CSV: {args.tracin_scores_csv}")
    print(f"Output dir: {output_dir}")
    print("=" * 60)

    # Create dataloaders with TracIn-Weighted Sampler
    train_loader, val_loader, train_sampler = create_tracin_weighted_dataloaders(
        ...
        tracin_scores_csv=args.tracin_scores_csv,
        alpha=args.alpha,
    ...
    loss_fn = MaskedCombinedLossV2(
        feature_weight=1.0,  # MSE on encoder features (exp_k v6 default)
        triplet_weight=1.0,  # Triplet loss (exp_k v6 default)
        triplet_margin=0.2,  # Triplet margin (exp_k v6 default)
    )
    ...
    # Save final summary
    summary = {
        'experiment': 'exp_0128_tracin_weighted_soft_reweighting',
        'alpha': args.alpha,
        'baseline': baseline,
        'final': metrics_final,
        'improvement': {
        ...
        'success': (
            ...
            metrics_final['strict_acc'] >= baseline['strict_acc'] * 0.9  # 允許 10% 下降
        ),
    }
