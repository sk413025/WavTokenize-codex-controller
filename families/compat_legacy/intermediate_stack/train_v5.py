"""
exp_0112_intermediate: Exp K v5 - 動態中間層監督訓練

V5 改進重點 (解決 V4 困難樣本導致中間層損失持續上升問題):
1. 延長 Curriculum 過渡期: 200 epochs (原 100)
2. 限制困難樣本比例: curriculum_end=0.85 (排除最困難 15%)
3. 動態中間層權重衰減: curriculum 完成後逐步降低 intermediate_weight
4. Warmdown 機制: 防止困難樣本導致中間層過擬合
...
- L4/model[4] (1.0): ResBlock, Cosine Loss
...
from families.deps.wavtokenizer_core.config import WAVTOK_CONFIG, WAVTOK_CKPT, DISTANCE_MATRIX, TRAIN_CACHE, VAL_CACHE
...
class IntermediateSupervisionLossV5(nn.Module):
    ...
    def __init__(self, layer_weights: dict = None, target_scale: float = 1.0):
        ...
        print(f"     Feature: Dynamic warmdown support")
...
def plot_training_curves_v5(history, save_path):
    """繪製 V5 訓練曲線"""
    fig, axes = plt.subplots(3, 3, figsize=(15, 12))
    fig.suptitle('Exp K v5: Dynamic Intermediate Supervision with Warmdown', fontsize=14)
    ...
    ax.set_title('V5: Dynamic Intermediate Weight')
    ax.set_xlabel('Epoch')
    ...
    ax.plot(epochs, history['train_intermediate_L4_loss'], 'g--', label='L4 (w=1.0)')
...
def main():
    parser = argparse.ArgumentParser(description='Exp K v5: Dynamic Intermediate Layer Supervision')

    # Experiment
    parser.add_argument('--exp_name', type=str, default='exp_k_v5')
    ...
    # V5: Dynamic Intermediate Supervision
    ...
    parser.add_argument('--curriculum_epochs', type=int, default=200,
                        help='Extended curriculum period (was 100 in V4)')
    ...
    if args.output_dir:
        ...
        run_dir = Path('families/compat_legacy/intermediate_stack/runs') / f"{args.exp_name}_{timestamp}"
    run_dir.mkdir(parents=True, exist_ok=True)

    # Save config
    config = vars(args)
    config['v5_info'] = {
        'description': 'Dynamic intermediate supervision with warmdown',
        'changes': [
            'Extended curriculum: 200 epochs (was 100)',
            'Limited hard samples: curriculum_end=0.85 (exclude hardest 15%)',
            'Dynamic intermediate weight: warmdown from 0.5 to 0.25',
            'Warmdown period: 50 epochs after curriculum ends',
            'Same layer config: L3(0.3) + L4(1.0) + L6(0.5)',
        ],
        'motivation': 'V4 showed intermediate loss kept increasing after hard samples added. '
                      'V5 addresses this by slower curriculum transition and weight warmdown.'
    }
    with open(run_dir / 'config.json', 'w') as f:
        json.dump(config, f, indent=2)

    print(f"\n{'='*60}")
    print(f"Exp K v5: Dynamic Intermediate Layer Supervision")
    print(f"{'='*60}")
    ...
    # V5: 更細緻的 phase 調整
    num_phases = int((args.curriculum_end - args.curriculum_start) / 0.05) + 1  # 0.05 step
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if epoch % 10 == 0 or epoch == args.num_epochs:
            plot_training_curves_v5(history, run_dir / 'training_curves.png')
