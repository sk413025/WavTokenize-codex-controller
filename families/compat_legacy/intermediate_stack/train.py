"""
exp_0112_intermediate: Exp N - 中間層監督訓練

核心設計:
- 在 L4, L8 加入中間層監督 (直接 MSE 比對)
- 讓噪音敏感層直接獲得學習信號
- 多尺度 Loss: 中間層 MSE + 最終輸出 Feature/Triplet Loss

Loss 設計:
  Total Loss = λ_inter × (Loss_L4 + Loss_L8) + Feature Loss + Triplet Loss

執行:
    python families/compat_legacy/intermediate_stack/train.py --exp_name exp_n_intermediate
"""
...
from families.compat_legacy.intermediate_stack.models import TeacherStudentIntermediate, IntermediateSupervisionLoss
...
def main():
    parser = argparse.ArgumentParser(description='Exp 0112_intermediate: 中間層監督 (Exp N)')

    # Experiment
    parser.add_argument('--exp_name', type=str, default='exp_n_intermediate')
    ...
    parser.add_argument('--intermediate_weight', type=float, default=0.5,
                        help='Weight for intermediate supervision loss')
