...
核心概念：
- 目前的 Feature Loss 優化的 latent space 不一定適合 VQ quantization
...
from families.compat_legacy.curriculum_data.losses import MaskedCombinedLossV3, compute_masked_accuracy
...
def train_epoch(model, dataloader, optimizer, loss_fn, device, epoch,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        vq_commit = loss_info.get('vq_commitment_loss', 0)
        pbar.set_postfix({
            'loss': f"{loss.item():.4f}",
...
def main():
    parser = argparse.ArgumentParser(description='Exp63: VQ-Aware Loss')

    # Experiment
    parser.add_argument('--exp_name', type=str, default='exp63_vq_aware')
    ...
    print("=" * 60)
    print(f"Exp63: VQ-Aware Loss")
    print(f"Experiment: {args.exp_name}")
    print("=" * 60)
    print(f"Loss Config:")
    print(f"  Feature: {args.feature_weight}")
    print(f"  Triplet: {args.triplet_weight} (margin={args.triplet_margin})")
    print(f"  VQ Commitment: {args.vq_commitment_weight} (NEW)")
    print(f"  VQ Distortion: {args.vq_distortion_weight} (NEW)")
    ...
    # Loss Function - 使用 V3 版本支援 VQ-Aware Loss
