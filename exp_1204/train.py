...
try:
    ...
    from .losses import CurriculumEmbDistillationLoss, EmbDistillationLoss
...
class Trainer:
    ...
    def __init__(self, config, exp_name):
        ...
        # 創建 loss function
        # exp_1204: 使用 CurriculumEmbDistillationLoss (漸進式學習 + 混合 Loss)
        use_curriculum_raw = getattr(config, 'use_curriculum', True)
        # 處理字串 "true"/"false" 或布林值
        if isinstance(use_curriculum_raw, str):
            self.use_curriculum = use_curriculum_raw.lower() == 'true'
        else:
            self.use_curriculum = bool(use_curriculum_raw)
        self.use_emb_distillation = True  # exp_1204 固定使用 emb_distillation

        if self.use_curriculum:
            # 使用 Curriculum Learning (漸進式 MSE + CE)
            self.criterion = CurriculumEmbDistillationLoss(
                mse_weight=getattr(config, 'mse_weight', 1.0),
                ce_weight=getattr(config, 'ce_weight', 0.5),  # 最終 CE 權重
                feature_loss_weight=config.feature_loss_weight,
                initial_temperature=getattr(config, 'initial_temperature', 2.0),
                final_temperature=getattr(config, 'final_temperature', 0.1),
                curriculum_mode=getattr(config, 'curriculum_mode', 'linear'),
                warmup_epochs=getattr(config, 'warmup_epochs', 5),
                transition_epochs=getattr(config, 'transition_epochs', 20),
                label_smoothing=getattr(config, 'label_smoothing', 0.0),
            )
            print(f"\n{'='*60}")
            print(f"exp_1204: Curriculum Learning + Mixed Loss")
            print(f"{'='*60}")
            print(f"  MSE weight: {getattr(config, 'mse_weight', 1.0)}")
            print(f"  CE weight (final): {getattr(config, 'ce_weight', 0.5)}")
            print(f"  Initial temperature: {getattr(config, 'initial_temperature', 2.0)}")
            print(f"  Final temperature: {getattr(config, 'final_temperature', 0.1)}")
            print(f"  Curriculum mode: {getattr(config, 'curriculum_mode', 'linear')}")
            print(f"  Warm-up epochs: {getattr(config, 'warmup_epochs', 5)}")
            print(f"  Transition epochs: {getattr(config, 'transition_epochs', 20)}")
            print(f"{'='*60}\n")
        else:
            # 使用原有的 EmbDistillationLoss (無 curriculum)
            self.criterion = EmbDistillationLoss(
                emb_to_codebook_weight=getattr(config, 'emb_to_codebook_weight', 1.0),
                ce_token_weight=getattr(config, 'ce_token_weight', 0.0),
                feature_loss_weight=config.feature_loss_weight,
                vq_loss_weight=config.vq_loss_weight,
                temperature=config.temperature,
                label_smoothing=getattr(config, 'label_smoothing', 0.0),
            )
            print(f"Distance loss mode: emb_distillation (直接監督 encoder 原始輸出)")
            print(f"  emb_to_codebook_weight: {getattr(config, 'emb_to_codebook_weight', 1.0)}")
            print(f"  ce_token_weight: {getattr(config, 'ce_token_weight', 0.0)}")
        ...
        self.history = {
            ...
            'train_mse_loss': [], 'train_ce_loss': [],  # exp_1204: 分別記錄
            'val_loss': [], 'val_feature_loss': [], 'val_distance_loss': [],
            'val_vq_loss': [], 'val_token_acc': [],
            'val_vq_feature_monitor': [],  # VQ後特徵差異監控
            'val_mse_loss': [], 'val_ce_loss': [],  # exp_1204: 分別記錄
            'epochs': [], 'learning_rates': [],
            # exp_1204: Curriculum 狀態
            'temperatures': [], 'ce_weights': []
        }
    ...
    def train_epoch(self, epoch):
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # exp_1204: 更新 Curriculum Learning 狀態
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        if self.use_curriculum and hasattr(self.criterion, 'set_epoch'):
            self.criterion.set_epoch(epoch, self.config.num_epochs)
            curr_temp = self.criterion.get_current_temperature()
            curr_ce = self.criterion.get_current_ce_weight()
            print(f"  📈 Curriculum: epoch={epoch}, temp={curr_temp:.3f}, ce_weight={curr_ce:.3f}")
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if self.global_step % self.config.log_interval == 0:
                ...
                self.writer.add_scalar('train/feature_loss', loss_dict.get('feature_loss', 0.0), self.global_step)
                self.writer.add_scalar('train/emb_to_codebook_loss', loss_dict.get('emb_to_codebook_loss', 0.0), self.global_step)
                ...
                self.writer.add_scalar('train/vq_loss', loss_dict.get('vq_loss', 0.0), self.global_step)
                self.writer.add_scalar('train/token_acc', loss_dict.get('code_match_rate', 0.0), self.global_step)
                # Curriculum Learning 專用指標
                self.writer.add_scalar('train/mse_loss', loss_dict.get('mse_loss', 0.0), self.global_step)
                self.writer.add_scalar('train/ce_loss', loss_dict.get('ce_loss', 0.0), self.global_step)
                self.writer.add_scalar('curriculum/temperature', loss_dict.get('current_temperature', 1.0), self.global_step)
                self.writer.add_scalar('curriculum/ce_weight', loss_dict.get('current_ce_weight', 0.0), self.global_step)
    ...
    def validate(self, epoch):
        ...
        for batch in tqdm(self.val_loader, desc="Validating"):
            ...
            val_loss += loss.item()
            val_feature_loss += loss_dict.get('feature_loss', loss_dict.get('emb_to_codebook_loss', 0.0))
            val_distance_loss += loss_dict.get('soft_distance_loss',
                                               loss_dict.get('emb_to_codebook_loss',
                                                            loss_dict.get('hard_distance_loss', 0.0)))
...
def main():
    ...
    parser.add_argument('--vq_loss_weight', type=float, default=0.0,
                       help='VQ loss weight (原始 commitment loss，保持 0)')
    parser.add_argument('--correct_vq_loss_weight', type=float, default=0.0,
                       help='Correct VQ loss weight (讓 features 接近 teacher 的 codebook embedding)')
    parser.add_argument('--temperature', type=float, default=1.0,
                       help='Softmax temperature for distance loss')
    parser.add_argument('--distance_loss_mode', type=str, default='gumbel',
                       choices=['soft', 'gumbel', 'ste', 'ce', 'margin', 'emb_distillation'],
                       help='Distance loss mode: soft, gumbel, ste, ce, margin, emb_distillation')
    ...
    # EmbDistillation 專用參數 (當 distance_loss_mode='emb_distillation' 時使用)
    parser.add_argument('--emb_to_codebook_weight', type=float, default=1.0,
                       help='Weight for emb-to-codebook loss (MSE: student_emb → codebook[teacher_codes])')
    parser.add_argument('--ce_token_weight', type=float, default=0.0,
                       help='Weight for CE token loss (optional auxiliary loss)')

    # Curriculum Learning 參數 (exp_1204)
    parser.add_argument('--use_curriculum', type=str, default='false',
                       help='Enable curriculum learning (true/false)')
    parser.add_argument('--mse_weight', type=float, default=1.0,
                       help='MSE Loss weight')
    parser.add_argument('--ce_weight', type=float, default=0.5,
                       help='CE Loss final weight (will increase from 0 to this value)')
    parser.add_argument('--initial_temperature', type=float, default=2.0,
                       help='Initial temperature for soft supervision')
    parser.add_argument('--final_temperature', type=float, default=0.1,
                       help='Final temperature for hard supervision')
    parser.add_argument('--curriculum_mode', type=str, default='linear',
                       choices=['linear', 'cosine', 'step'],
                       help='Curriculum progression mode')
    parser.add_argument('--warmup_epochs', type=int, default=5,
                       help='Number of warm-up epochs (MSE only)')
    parser.add_argument('--transition_epochs', type=int, default=20,
                       help='Number of transition epochs')
