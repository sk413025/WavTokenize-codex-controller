...
def detect_and_trim_speech(wav, threshold=0.03, min_silence_duration=0.25, target_sr=24000):
    """檢測語音活躍區域並裁剪
    
    Args:
        wav: 輸入音頻張量 [1, T]
        threshold: 能量閾值，低於此值視為靜音
        min_silence_duration: 最小靜音持續時間(秒)，低於此值的靜音不計
        target_sr: 採樣率
    ...
    try:
        ...
        frame_length = int(0.025 * target_sr)  # 25ms窗口
        hop_length = int(0.010 * target_sr)    # 10ms步長
        
        energy = np.array([
            np.sum(wav_np[i:i+frame_length]**2) 
        ...
        # 將短暫的靜音視為語音的一部分（平滑處理）
        min_silence_frames = int(min_silence_duration / (hop_length / target_sr))
        ...
        for i, speech in enumerate(is_speech):
            if not in_speech and speech:
                ...
                # 可能是語音結束
                if i - speech_start >= min_silence_frames:
                    in_speech = False
                    # 轉換回採樣點位置
                    start_sample = speech_start * hop_length
                    end_sample = i * hop_length + frame_length
                    # 添加語音塊
                    speech_chunks.append((start_sample, end_sample))
        
        # 如果音頻結尾是語音
        if in_speech:
            start_sample = speech_start * hop_length
            end_sample = len(wav_np)
            speech_chunks.append((start_sample, end_sample))
        ...
        # 選擇最長的語音塊
        longest_chunk = max(speech_chunks, key=lambda x: x[1] - x[0])
        start, end = longest_chunk
        
        # 確保不超出邊界
        start = max(0, start)
        end = min(len(wav_np), end)
        ...
        print(f"語音檢測錯誤: {str(e)}")
        # 發生錯誤時返回原始音頻
        return wav

def calculate_speech_rate(audio_path, target_sr=24000):
    """計算語音的語速，使用音節或發音單元數量除以持續時間
    ...
    try:
        ...
        # 使用能量變化次數作為語速指標
        energy_diff = np.diff(energy > np.mean(energy) * 0.1)
        changes = np.sum(np.abs(energy_diff))
        
        # 計算持續時間（秒）
        duration = len(wav_np) / target_sr
        
        # 計算變化率（變化次數/秒）
        speech_rate = changes / duration
        
        return speech_rate
        
    except Exception as e:
        print(f"計算語速時出錯: {str(e)}")
        return None
        ...
        if self.max_sentences_per_speaker is not None:
            ...
            for key, pairs in speaker_groups.items():
                if len(pairs) > self.max_sentences_per_speaker:
                    ...
                    # 按照content_id排序，選擇前N句（編號1-100）
                    sorted_pairs = sorted(pairs, key=lambda x: int(x['content_id']))
                    selected_pairs = sorted_pairs[:self.max_sentences_per_speaker]
                    print(f"選擇 {key} 的編號 {selected_pairs[0]['content_id']}-{selected_pairs[-1]['content_id']}")
...
def collate_fn(batch):
    ...
    content_ids = [item[2] for item in batch]  # 提取內容ID
