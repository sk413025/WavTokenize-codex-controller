...
# 用法: bash exp_0305c/wait_and_launch_exp0304.sh &

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
EXPC_LOG="${ROOT_DIR}/exp_0305c/nohup_expc_20260306_090111.log"
...
while true; do
    if grep -qE "^Training complete" "${EXPC_LOG}" 2>/dev/null; then
        echo "[$(date)] exp_0305c 完成，準備啟動 exp_0304..."
        break
    fi
...
LOG_EXP0304="${ROOT_DIR}/exp_0304/nohup_material_gen_${TS}.log"
echo "[$(date)] 啟動 exp_0304 材質泛化 (cuda:1) => ${LOG_EXP0304}"

cd "${ROOT_DIR}"
PYTHONPATH=/home/sbplab/ruizi/WavTokenize-self-supervised:$PYTHONPATH \
nohup /home/sbplab/miniconda3/envs/test/bin/python exp_0304/train_material_gen.py \
    --mode epoch \
    --epochs 300 \
    --device cuda:1 \
    --batch_size 8 \
...
echo "${PID}" > "${ROOT_DIR}/exp_0304/material_gen_pid.txt"
echo "[$(date)] 完成！監控："
