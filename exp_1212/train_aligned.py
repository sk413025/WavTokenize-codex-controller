...
修復兩個對齊問題:
1. 來源1: Per-pair mismatch → 使用 min-length truncation
2. 來源2: Cross-sample mismatch → 使用 masked loss

架構:
    Noisy Audio → Encoder(LoRA) → VQ(凍結) → tokens
"""
...
def compute_unmasked_accuracy(student_codes, teacher_codes):
    """計算未 mask 的 token accuracy (for comparison)"""
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        lengths = batch['lengths'].to(device)  # 新增: 有效長度
        ...
        # Unmasked Accuracy (for comparison)
...
def plot_metrics(history, exp_dir):
    ...
    # Unmasked Accuracy (比較)
    ax = axes[0, 2]
    ax.plot([x * 100 for x in history['train_unmasked_acc']], label='Train Unmasked')
    ax.plot([x * 100 for x in history['val_unmasked_acc']], label='Val Unmasked')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Accuracy (%)')
    ax.set_title('Unmasked Token Accuracy (Comparison)')
    ax.legend()
    ax.grid(True)
    ...
    # Masked vs Unmasked Accuracy Gap
    ax = axes[1, 2]
    train_gap = [m - u for m, u in zip(history['train_masked_acc'], history['train_unmasked_acc'])]
    val_gap = [m - u for m, u in zip(history['val_masked_acc'], history['val_unmasked_acc'])]
    ax.plot([x * 100 for x in train_gap], label='Train Gap')
    ax.plot([x * 100 for x in val_gap], label='Val Gap')
    ax.axhline(y=0, color='k', linestyle='--', alpha=0.3)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Gap (%)')
    ax.set_title('Masked - Unmasked Accuracy Gap')
    ax.legend()
    ax.grid(True)
...
def main():
    ...
    # Scheduler 參數
    parser.add_argument('--use_scheduler', action='store_true', help='Use cosine scheduler')
    ...
    print("=" * 60)
    print(f"exp_1212: 對齊修復版訓練")
    print(f"Experiment: {args.exp_name}")
    print("=" * 60)
    print("修復內容:")
    print("  1. Per-pair min-length truncation")
    print("  2. Masked loss (忽略 padding)")
    print("=" * 60)

    # 載入資料 (使用對齊版)
    print("\n載入資料 (Aligned Dataset)...")
    ...
    print(f"  Encoder stride: {args.encoder_stride}")
    ...
    if args.use_scheduler:
        ...
        cosine_scheduler = CosineAnnealingLR(
            ...
            eta_min=args.lr * 0.01  # 最低 lr = 1% of initial
        )
        # Combine
