...
def test_model(input_path, output_path, model_config):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            print("Features range:", features.min().item(), "to", features.max().item())
    
            # 特徵增強
            features = features * model_config['feature_scale']
            features = torch.tanh(features)
            print("Enhanced features range:", features.min().item(), "to", features.max().item())
    
            # 生成T-SNE圖 (在解碼前)
            tsne_path = os.path.join(os.path.dirname(output_path), 
                                   f"tsne_{Path(input_path).stem}.png")
            visualize_tsne(features.cpu().squeeze(), tsne_path, Path(input_path).name)
            ...
            print("Final output range:", output.min().item(), "to", output.max().item())
    ...
    for idx, wav_path in enumerate(wav_files, 1):
        try:
            ...
            with torch.no_grad():
                ...
                # 生成T-SNE圖 (在解碼前)
                tsne_path = os.path.join(output_dir, 'tsne_plots', 
                                       f"tsne_{Path(wav_path).stem}.png")
                visualize_tsne(features.cpu().squeeze(), tsne_path, Path(wav_path).name)
                print(f"Saved T-SNE plot to: {tsne_path}")
...
if __name__ == "__main__":
    config = {
        ...
        #'checkpoint_path': "./self_output2/best_model.pth",
        'checkpoint_path': "./tout/best_model.pth",
        'feature_scale': 1.5
    }
